...
import type { CalendarContext, RunResultSource } from './types'
...
export function resultDistanceKm(
	...
	return isJuniorEvent(result.event, eventName)
...
export function journeyMilestones(
	...
	for (const result of results) {
		kmByDate.set(
			result.date,
			(kmByDate.get(result.date) ?? 0) + resultDistanceKm(result, eventName),
		)
	}
...
export function journeyMilestonesByDate(
	...
	for (const milestone of journeyMilestones(results, eventName)) {
...
export function journeyMilestoneDetail(waypoint: JourneyWaypoint): string {
	...
	return `The total distance members have run has exceeded ${groupThousands(waypoint.km)}km which would take us from Stockholm to ${place}`
}
...
export function journeyMilestoneShortDetail(waypoint: JourneyWaypoint): string {
	return `Total distance > ${groupThousands(waypoint.km)}km`
}
