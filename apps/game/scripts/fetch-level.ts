...
if (!eventId) {
	console.error(
		'Usage: pnpm game:level <eventId> [--name "Display Name"] [--type=water|altitude|course|buildings|paths|roads|all]',
	)
	process.exit(1)
}
...
if (!VALID_TYPES.includes(fetchType)) {
	console.error(
		`Invalid --type="${fetchType}". Valid values: ${VALID_TYPES.join(', ')}`,
	)
	process.exit(1)
}
...
function filterWaterByDistance(
	...
	if (filtered.length < before) {
		console.log(
			`  Filtered water: ${before} → ${filtered.length} (within ${maxDist}m of path)`,
		)
	}
	return filtered
}
...
async function fetchCourse(eventId: string): Promise<CourseData> {
	...
	if (!res.ok) {
		throw new Error(
			`Failed to fetch course "${eventId}": ${res.status} ${res.statusText}`,
		)
	}
	return res.json()
}
	...
	try {
		...
		for (const endpoint of OVERPASS_ENDPOINTS) {
			try {
				...
				if (res.ok) break
				console.warn(
					`  ⚠ ${endpoint} responded with ${res.status}, trying next …`,
				)
			} catch (err) {
...
function writeLevelFile(
	...
	// Write each data type as a typed .ts file
	writeTypedData(resolve(levelDir, 'course.ts'), cleanCourse, 'LevelCourse', [
		'LevelCourse',
	])
	writeTypedData(
	...
	writeTypedData(resolve(levelDir, 'water.ts'), water, 'LevelWaterFeature[]', [
		'LevelWaterFeature',
	])
	...
	if (paths.length > 0) {
		writeTypedData(resolve(levelDir, 'paths.ts'), paths, "LevelData['paths']", [
			'LevelData',
		])
	}
	if (extras?.marshals && extras.marshals.length > 0) {
		writeTypedData(
			resolve(levelDir, 'marshals.ts'),
			extras.marshals,
			"LevelData['marshals']",
			['LevelData'],
		)
...
function regenerateIndex(levelNames: Record<string, string>) {
	...
	const subfolderIds = entries
		.filter(
			(e) =>
				e.isDirectory() && existsSync(resolve(LEVELS_DIR, e.name, 'index.ts')),
		)
		.map((e) => e.name)
		.sort()
	...
	console.log(
		`✅ Regenerated ${indexPath} (${allIds.length} level${allIds.length === 1 ? '' : 's'}: ${subfolderIds.length} migrated, ${legacyIds.length} legacy)`,
	)
}
...
async function readExistingLevel(id: string): Promise<ExistingLevel | null> {
	...
	if (existsSync(subfolderIndex)) {
		...
		if (
			existsSync(courseFile) &&
			existsSync(altFile) &&
			existsSync(waterFile)
		) {
			try {
				...
				const buildings = existsSync(buildingsFile)
					? JSON.parse(readFileSync(buildingsFile, 'utf-8'))
					: undefined
				const paths = existsSync(pathsFile)
					? JSON.parse(readFileSync(pathsFile, 'utf-8'))
					: undefined
				const marshals = existsSync(marshalsFile)
					? JSON.parse(readFileSync(marshalsFile, 'utf-8'))
					: undefined
				const roads = existsSync(roadsFile)
					? JSON.parse(readFileSync(roadsFile, 'utf-8'))
					: undefined
...
async function main() {
	...
	const [altitude, rawWater, rawBuildings, rawPaths, rawRoads] =
		await Promise.all([
			needAltitude
	...
	if (!isPartial) {
		...
		for (const e of entries) {
			if (
				e.isDirectory() &&
				e.name !== eventId &&
				existsSync(resolve(LEVELS_DIR, e.name, 'index.ts'))
			) {
				try {
		...
		for (const e of entries) {
			if (
				e.isFile() &&
				e.name.endsWith('.map.ts') &&
				!allNames[e.name.replace('.map.ts', '')]
			) {
