...
function updateAltitudeInFile(eventId: string, altitudes: number[]): void {
	...
	if (updated === content) {
		console.error(
			'Could not find altitude array in map file — no changes written.',
		)
		process.exit(1)
	}
...
if (args.length === 0) {
	console.error(
		'Usage: pnpm tsx scripts/apply-gpx-altitude.ts <gpxFile> [eventId]',
	)
	process.exit(1)
}
...
const altitudes = coordinates.map(([lon, lat]) =>
	nearestElevation(gpxPoints, lon, lat),
)
