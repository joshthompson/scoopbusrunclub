...
function sampleTrack(allPoints: GpxPoint[]): GpxPoint[] {
	...
	if (totalAvail < TARGET_DIST) {
		throw new Error(
			`GPX only has ${totalAvail.toFixed(0)}m, need ${TARGET_DIST}m`,
		)
	}
...
async function fetchBuildings(
	...
	const lonPad =
		paddingM / (111_000 * Math.cos((((minLat + maxLat) / 2) * Math.PI) / 180))
	minLat -= latPad
	maxLat += latPad
	minLon -= lonPad
	maxLon += lonPad
	...
	try {
		...
		if (!res.ok) {
			console.warn(`  ⚠ Overpass ${res.status}`)
			return []
		}
...
async function main() {
	...
	console.log(
		`  Sampled: ${sampled.length} points (every ${SAMPLE_INTERVAL}m for ${TARGET_DIST}m)`,
	)
