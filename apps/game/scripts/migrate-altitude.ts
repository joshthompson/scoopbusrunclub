...
function migrateLevel(levelId: string): [number, number, number][] {
	...
	if (!existsSync(coursePath) || !existsSync(altitudePath)) {
		console.log(
			`  ⏭ Skipping ${levelId} — missing course.json or altitude.json`,
		)
		return []
	}
	...
	if (altValues.length !== course.coordinates.length) {
		console.warn(
			`  ⚠ ${levelId}: altitude (${altValues.length}) ≠ coordinates (${course.coordinates.length})`,
		)
	}

	// Build new altitude: [lat, lon, alt][]
	const newAltitude: [number, number, number][] = course.coordinates.map(
		(coord, i) => {
			const lon = coord[0]
			const lat = coord[1]
...
function generateHagaAltCourse(
	existingAltitude: [number, number, number][],
): void {
	...
	console.log(
		`  Altitude merge: ${added} added, ${skipped} skipped (within 15m)`,
	)
