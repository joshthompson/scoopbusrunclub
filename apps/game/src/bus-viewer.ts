...
async function init() {
	...
	// Ground grid
	gridMesh = MeshBuilder.CreateGround(
		'grid',
		{ width: 40, height: 40, subdivisions: 40 },
		scene,
	)
	...
	const solidGround = MeshBuilder.CreateGround(
		'solidGround',
		{ width: 40, height: 40 },
		scene,
	)
	...
	const lfMesh = MeshBuilder.CreateSphere(
		'lightFront',
		{ diameter: 0.4 },
		scene,
	)
	...
	passengerTrayMarker = MeshBuilder.CreateBox(
		'passengerTray',
		{
...
function createAxes(scene: Scene, parent: TransformNode) {
	...
	for (const { axis, color } of colors) {
		const line = MeshBuilder.CreateLines(
			`axis_${axis.x}_${axis.y}_${axis.z}`,
			{
	...
	const labels = ['X', 'Y', 'Z']
	const positions = [
		new Vector3(len + 0.5, 0, 0),
		new Vector3(0, len + 0.5, 0),
		new Vector3(0, 0, len + 0.5),
	]
	for (let i = 0; i < 3; i++) {
...
function setupUI() {
	...
	// Scoop anim toggle
	const scoopAnimEl = document.getElementById(
		'tog-scoop-anim',
	) as HTMLInputElement
	scoopAnimEl?.addEventListener('change', () => {
		...
		if (!scoopAnimEnabled) {
			...
			scoopPivot.rotation.x = -(
				(Number.parseInt(slider.value, 10) * Math.PI) /
				180
			)
		}
	})
...
function generateExportText(): string {
	const fmt = (arr: number[]) =>
		`[${arr.map((v) => v.toFixed(2).replace(/\.?0+$/, '') || '0').join(', ')}]`
	const fmtSize = (arr: number[]) =>
		`[${arr.map((v) => v.toFixed(1).replace(/\.?0+$/, '') || '0').join(', ')}]`

	const scoop = [
		getNumVal('pos-scoop-x'),
		getNumVal('pos-scoop-y'),
		getNumVal('pos-scoop-z'),
	]
...
function applyPositions() {
	// Scoop
	scoopPivot.position.set(
		getNumVal('pos-scoop-x'),
		getNumVal('pos-scoop-y'),
		getNumVal('pos-scoop-z'),
	)
	// Scoop width — scale the scoop root proportionally to the new width
	const scoopGlbRoot = scoopPivot
		.getChildren(undefined, true)
		.find((c) => c.name === 'scoopGlbRoot') as TransformNode | undefined
	if (scoopGlbRoot) {
	...
	// Wheels - front
	frontWheelLeft.position.set(
		getNumVal('pos-wfl-x'),
		getNumVal('pos-wfl-y'),
		getNumVal('pos-wfl-z'),
	)
	frontWheelRight.position.set(
		getNumVal('pos-wfr-x'),
		getNumVal('pos-wfr-y'),
		getNumVal('pos-wfr-z'),
	)
	...
	// Wheels - back
	const rearPivots = busRoot
		.getChildren()
		.filter((c) => c.name.startsWith('rearWheelPivot'))
	if (rearPivots.length >= 2) {
		...
		if (backRight)
			(backRight as TransformNode).position.set(
				getNumVal('pos-wbr-x'),
				getNumVal('pos-wbr-y'),
				getNumVal('pos-wbr-z'),
			)
		if (backLeft)
			(backLeft as TransformNode).position.set(
				getNumVal('pos-wbl-x'),
				getNumVal('pos-wbl-y'),
				getNumVal('pos-wbl-z'),
			)
	...
	// Passenger tray
	passengerTrayMarker.position.set(
		getNumVal('pos-pt-x'),
		getNumVal('pos-pt-y'),
		getNumVal('pos-pt-z'),
	)
	...
	passengerTrayMarker.scaling.set(
		trayW / PASSENGER_TRAY_SIZE[0],
		1,
		trayL / PASSENGER_TRAY_SIZE[1],
	)
