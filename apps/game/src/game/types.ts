...
export interface Runner {
	...
	targetIdx: number
	/** True after finishing, while running back toward path index 0 */
...
export type BuildingFootprint = {
	type: 'grey' | 'red' | 'blue' | 'green' | 'yellow' | 'kristineberg' | 'brick'
	height?: number
...
export interface Swan {
	model: import('./objects/SwanModel').SwanModelResult
	mesh: Mesh
	x: number
	z: number
	yaw: number
