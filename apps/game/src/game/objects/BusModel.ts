...
export const WHEEL_FRONT_LEFT_POSITION: [number, number, number] = [
	-1.75, 0.8, 3.1,
]
export const WHEEL_FRONT_RIGHT_POSITION: [number, number, number] = [
	1.75, 0.8, 3.1,
]
export const WHEEL_BACK_LEFT_POSITION: [number, number, number] = [
	-1.75, 0.8, -3.3,
]
export const WHEEL_BACK_RIGHT_POSITION: [number, number, number] = [
	1.75, 0.8, -3.3,
]
export const LIGHT_FRONT_POSITION: [number, number, number] = [0, 1.5, 6]
export const LIGHT_BACK_LEFT_POSITION: [number, number, number] = [
	-1.5, 1.2, -5,
]
export const LIGHT_BACK_RIGHT_POSITION: [number, number, number] = [
	1.5, 1.2, -5,
]
...
export async function createBusModel(scene: Scene): Promise<BusModelResult> {
	...
	// ═══════════════════════════════════════
	const busResult = await SceneLoader.ImportMeshAsync(
		'',
		'',
		busModelUrl,
		scene,
	...
	for (let i = 0; i < wheelPositions.length; i++) {
		...
		const wheelResult = await SceneLoader.ImportMeshAsync(
			'',
			'',
			wheelModelUrl,
			scene,
	...
	// Load GLB model
	const scoopResult = await SceneLoader.ImportMeshAsync(
		'',
		'',
		scoopModelUrl,
		scene,
...
export const PLAYER_COLORS: BusColorPalette[] = BUS_COLOR_OPTIONS.map(
	(opt) => ({
 ...
 */
export function busColorPaletteFromOption(opt: {
	bodyHex: string
	scoopHex?: string
}): BusColorPalette {
	return {
 ...
 */
export function tintBusModel(
	root: TransformNode,
	palette: BusColorPalette,
	suffix: string,
) {
	...
	const scoopPivotNode = root
		...
		.find((c) => c.name === 'busBodyShell')
	if (scoopPivotNode) {
		const scoopPivot = scoopPivotNode
			.getChildren(undefined, false)
			.find((c) => c.name === 'scoopPivot')
		if (scoopPivot) {
