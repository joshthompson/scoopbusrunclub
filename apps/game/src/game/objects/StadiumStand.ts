...
export function createStadiumStand(
	...
	whiteMat.diffuseColor = new Color3(0.94, 0.93, 0.9)
	...
	steelMat.diffuseColor = new Color3(0.5, 0.52, 0.55)
	...
	windowMat.diffuseColor = new Color3(0.3, 0.42, 0.55)
	...
	for (let w = 0; w < numWins; w++) {
		...
		sill.position = new Vector3(
			wx,
			groundFloorH * 0.55 - winH * 0.5 - 0.04,
			halfD + 0.08,
		)
	...
	const trimStrip = MeshBuilder.CreateBox(
		'stadiumTrim',
		{
			width: width + 0.1,
			height: 0.12,
	...
	const backFill = MeshBuilder.CreateBox(
		'stadiumBackFill',
		{
			width: width * 0.94,
	...
	railBar.position = new Vector3(
		0,
		railY,
		seatingFrontZ + rowStepD * 0.5 + 0.15,
	)
