...
const NECK_BLACK = new Color3(0.1, 0.1, 0.1)
const CHINSTRAP = new Color3(0.95, 0.95, 0.95) // white chinstrap
const BREAST_WHITE = new Color3(0.92, 0.9, 0.86) // warm white breast
...
const EYE_RING = new Color3(0.92, 0.9, 0.85) // faint white ring around eye
...
export function createGooseModel(scene: Scene, id: number): GooseModelResult {
	...
	const breast = MeshBuilder.CreateBox(
		`gBreast_${id}`,
		{
			width: BODY_W - 0.04,
			height: BREAST_H,
	...
	breast.position.set(
		0,
		LEG_H + BREAST_H / 2 + 0.01,
		BODY_D / 2 - BREAST_D / 2 - 0.02,
	)
	breast.parent = root
	...
	for (const side of [-1, 1]) {
		...
		const ring = MeshBuilder.CreateBox(
			`gEyeR${side > 0 ? 'R' : 'L'}_${id}`,
			{
				width: 0.01,
				height: eyeSize + 0.006,
				depth: eyeSize + 0.006,
			},
			scene,
		)
	...
	beak.position.set(
		0,
		headLocalY - 0.015,
		headLocalZ + HEAD_D / 2 + BEAK_D / 2 - 0.01,
	)
