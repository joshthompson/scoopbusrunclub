...
function makeCloudTexture(
	...
	for (let i = 0; i < shadeBlobs; i++) {
		...
		const rx = width * (0.1 + rand() * 0.18)
		const ry = height * (0.06 + rand() * 0.1)
...
function createClouds(scene: Scene): Mesh[] {
	...
	for (let i = 0; i < CLOUD_COUNT; i++) {
		...
		for (let layer = 0; layer < LAYERS_PER_CLOUD; layer++) {
			...
			const layerAlpha = layer === 0 ? 0.8 : 0.35 + rand() * 0.15
			...
			const layerOffsetY = (layer - 1) * h * 0.1
