...
function getWoodMat(scene: Scene) {
	if (!_woodMat) {
		_woodMat = new StandardMaterial('objWood', scene)
		_woodMat.diffuseColor = new Color3(0.48, 0.32, 0.18)
		_woodMat.specularColor = new Color3(0.06, 0.04, 0.02)
	}
	return _woodMat
}
function getIronMat(scene: Scene) {
	if (!_ironMat) {
		_ironMat = new StandardMaterial('objIron', scene)
		_ironMat.diffuseColor = new Color3(0.18, 0.18, 0.2)
		_ironMat.specularColor = new Color3(0.1, 0.1, 0.12)
	}
	return _ironMat
}
function getGreenMat(scene: Scene) {
	if (!_greenMat) {
		_greenMat = new StandardMaterial('objGreen', scene)
		_greenMat.diffuseColor = new Color3(0.15, 0.35, 0.12)
		_greenMat.specularColor = Color3.Black()
	}
	return _greenMat
}
function getWhiteMat(scene: Scene) {
	if (!_whiteMat) {
		_whiteMat = new StandardMaterial('objWhite', scene)
		_whiteMat.diffuseColor = new Color3(0.92, 0.92, 0.92)
		_whiteMat.specularColor = new Color3(0.1, 0.1, 0.1)
	}
	return _whiteMat
}
function getNetMat(scene: Scene) {
	if (!_netMat) {
		_netMat = new StandardMaterial('objNet', scene)
		_netMat.diffuseColor = new Color3(0.85, 0.85, 0.85)
		_netMat.specularColor = Color3.Black()
		_netMat.alpha = 0.6
	}
	return _netMat
}
function getCourtMat(scene: Scene) {
	if (!_courtMat) {
		_courtMat = new StandardMaterial('objCourt', scene)
		_courtMat.diffuseColor = new Color3(0.22, 0.42, 0.22)
		_courtMat.specularColor = Color3.Black()
	}
	return _courtMat
}
function getGlassMat(scene: Scene) {
	if (!_glassMat) {
		_glassMat = new StandardMaterial('objGlass', scene)
		_glassMat.diffuseColor = new Color3(0.85, 0.82, 0.6)
		_glassMat.emissiveColor = new Color3(0.35, 0.32, 0.15)
		_glassMat.specularColor = new Color3(0.2, 0.2, 0.1)
		_glassMat.alpha = 0.85
	}
	return _glassMat
}
...
function createBenchTemplates(scene: Scene): BenchTemplates {
	...
	const seatSlat = MeshBuilder.CreateBox(
		'tpl_bench_seat',
		{ width: B_LEN, height: B_SEAT_T, depth: B_SLAT_D },
		scene,
	)
	seatSlat.material = wm
	seatSlat.isVisible = false

	const backSlat = MeshBuilder.CreateBox(
		'tpl_bench_back',
		{ width: B_LEN, height: B_BACK_SLAT_H, depth: B_BACK_T },
		scene,
	)
	backSlat.material = wm
	backSlat.isVisible = false

	const frontLeg = MeshBuilder.CreateBox(
		'tpl_bench_fl',
		{ width: B_LEG_W, height: B_SEAT_H, depth: B_LEG_D },
		scene,
	)
	frontLeg.material = im
	frontLeg.isVisible = false

	const rlH = B_SEAT_H + B_BACK_H
	const rearLeg = MeshBuilder.CreateBox(
		'tpl_bench_rl',
		{ width: B_LEG_W, height: rlH, depth: B_LEG_D },
		scene,
	)
	rearLeg.material = im
	rearLeg.isVisible = false

	const crossbar = MeshBuilder.CreateBox(
		'tpl_bench_cb',
		{ width: B_LEG_W, height: B_LEG_D, depth: B_SEAT_D * 0.8 },
		scene,
	)
	crossbar.material = im
	crossbar.isVisible = false
...
function placeBench(
	...
	for (let si = 0; si < 3; si++) {
		...
		inst.position.set(
			0,
			B_SEAT_H,
			-B_SEAT_D / 2 + B_SLAT_D / 2 + si * (B_SLAT_D + B_SLAT_GAP),
		)
		inst.parent = root
	}
	...
	for (let bi = 0; bi < 2; bi++) {
		...
		inst.position.set(
			...
			B_SEAT_H +
				...
				0.06 +
				B_BACK_SLAT_H / 2 +
				bi * (B_BACK_SLAT_H + B_BACK_SLAT_GAP),
			-B_SEAT_D / 2,
		)
...
function createLampTemplates(scene: Scene): LampTemplates {
	...
	const base = MeshBuilder.CreateCylinder(
		...
		{
			height: 0.2,
			diameterTop: LP_POLE_D * 1.5,
			diameterBottom: LP_POLE_D * 2.8,
			tessellation: 8,
		},
		scene,
	)
	...
	const ring = MeshBuilder.CreateCylinder(
		...
		{
			height: 0.12,
			diameterTop: LP_POLE_D * 1.3,
			diameterBottom: LP_POLE_D * 1.5,
			tessellation: 8,
		},
		scene,
	)
	...
	const pole = MeshBuilder.CreateCylinder(
		'tpl_lamp_pole',
		{
			height: LP_POLE_H - 0.8,
			diameterTop: LP_POLE_D * 0.75,
			diameterBottom: LP_POLE_D,
			tessellation: 8,
		},
		scene,
	)
	...
	const collar = MeshBuilder.CreateCylinder(
		...
		{
			height: 0.08,
			diameterTop: LP_POLE_D * 1.6,
			diameterBottom: LP_POLE_D * 1.0,
			tessellation: 8,
		},
		scene,
	)
	collar.material = im
	collar.isVisible = false

	const lantern = MeshBuilder.CreateCylinder(
		'tpl_lamp_lantern',
		{
			height: LP_LANTERN_H,
			diameterTop: LP_LANTERN_D * 0.7,
			diameterBottom: LP_LANTERN_D,
			tessellation: 6,
		},
		scene,
	)
	...
	const roof = MeshBuilder.CreateCylinder(
		'tpl_lamp_roof',
		{
			height: 0.1,
			diameterTop: 0.06,
			diameterBottom: LP_LANTERN_D * 1.1,
			tessellation: 6,
		},
		scene,
	)
	...
	const spike = MeshBuilder.CreateCylinder(
		'tpl_lamp_spike',
		{ height: 0.15, diameterTop: 0, diameterBottom: 0.05, tessellation: 6 },
		scene,
	)
	spike.material = im
...
function createTennisTemplates(scene: Scene): TennisTemplates {
	...
	const surface = MeshBuilder.CreateBox(
		'tpl_tc_surf',
		{ width: TC_W, height: 0.02, depth: TC_L },
		scene,
	)
	surface.material = cm
	...
	const baseline = MeshBuilder.CreateBox(
		'tpl_tc_bl',
		{ width: TC_W + 0.06, height: 0.005, depth: 0.06 },
		scene,
	)
	baseline.material = wm
	baseline.isVisible = false

	const sideline = MeshBuilder.CreateBox(
		'tpl_tc_sl',
		{ width: 0.06, height: 0.005, depth: TC_L },
		scene,
	)
	sideline.material = wm
	sideline.isVisible = false

	const centerLine = MeshBuilder.CreateBox(
		'tpl_tc_cl',
		{ width: 0.06, height: 0.005, depth: TC_L * 0.54 },
		scene,
	)
	centerLine.material = wm
	centerLine.isVisible = false

	const serviceLine = MeshBuilder.CreateBox(
		'tpl_tc_svl',
		{ width: TC_W / 2 + 0.06, height: 0.005, depth: 0.06 },
		scene,
	)
	serviceLine.material = wm
	serviceLine.isVisible = false

	const net = MeshBuilder.CreateBox(
		'tpl_tc_net',
		{ width: TC_W + 0.5, height: TC_NET_H, depth: 0.03 },
		scene,
	)
	net.material = nm
	net.isVisible = false
	...
	const post = MeshBuilder.CreateCylinder(
		'tpl_tc_post',
		{ height: TC_NET_H + 0.15, diameter: 0.06, tessellation: 8 },
		scene,
	)
	post.material = im
