...
const HEAD_BROWN = new Color3(0.50, 0.32, 0.16);
const NOSE_DARK = new Color3(0.15, 0.10, 0.08);
const EYE_COLOR = new Color3(0.06, 0.06, 0.06);
const EYE_RING = new Color3(0.85, 0.78, 0.60);
...
export function createDeerModel(
  scene: Scene,
  id: number,
): DeerModelResult {
  ...
  //  Body
  // ═══════════════════════════════════════
  const body = MeshBuilder.CreateBox(`dBody_${id}`, {
  ...
  const belly = MeshBuilder.CreateBox(`dBelly_${id}`, {
    width: BODY_W - 0.04, height: BELLY_H, depth: BELLY_D,
  }, scene);
  ...
  //  Tail
  // ═══════════════════════════════════════
  const tail = MeshBuilder.CreateBox(`dTail_${id}`, {
  ...
  const neck = MeshBuilder.CreateBox(`dNeck_${id}`, {
  ...
  //  Head
  // ═══════════════════════════════════════
  const head = MeshBuilder.CreateBox(`dHead_${id}`, {
  ...
  // Nose
  const nose = MeshBuilder.CreateBox(`dNose_${id}`, {
  ...
  for (const side of [-1, 1]) {
    const ring = MeshBuilder.CreateBox(`dEyeR${side > 0 ? 'R' : 'L'}_${id}`, {
      width: 0.01, height: eyeSize + 0.008, depth: eyeSize + 0.008,
    }, scene);
    ring.material = eyeRingMat;
    ring.position.set(side * (HEAD_W / 2 + 0.001), eyeY, HEAD_Z + HEAD_D / 4);
    ring.parent = neckPivot;

    const eye = MeshBuilder.CreateBox(`dEye${side > 0 ? 'R' : 'L'}_${id}`, {
  ...
  // Ears
  for (const side of [-1, 1]) {
    const ear = MeshBuilder.CreateBox(`dEar${side > 0 ? 'R' : 'L'}_${id}`, {
      width: EAR_W, height: EAR_H, depth: EAR_D,
    }, scene);
    ear.material = earInnerMat;
    ear.position.set(side * (HEAD_W / 2 - 0.01), HEAD_Y + HEAD_H / 2 + EAR_H / 2 - 0.01, HEAD_Z - 0.02);
  ...
  if (id % 2 === 0) {
    for (const side of [-1, 1]) {
      // Main antler shaft
      const shaft = MeshBuilder.CreateBox(`dAntler${side > 0 ? 'R' : 'L'}_${id}`, {
        width: ANTLER_W, height: ANTLER_H, depth: ANTLER_D,
      }, scene);
      ...
      // Branch
      const branch = MeshBuilder.CreateBox(`dAntlerBr${side > 0 ? 'R' : 'L'}_${id}`, {
        width: ANTLER_BRANCH_W, height: ANTLER_BRANCH_H, depth: ANTLER_BRANCH_D,
      }, scene);
...
function buildLeg(prefix: string, pivot: TransformNode, mat: StandardMaterial, scene: Scene) {
  const leg = MeshBuilder.CreateBox(`${prefix}Leg`, {
  ...
  const hoof = MeshBuilder.CreateBox(`${prefix}Hoof`, {
    width: HOOF_W, height: HOOF_H, depth: HOOF_D,
  }, scene);
  hoof.material = mat;
