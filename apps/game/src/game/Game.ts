...
export class Game {
	...
	private pathTypeSegments: PathTypeSegment[] = []
	private pathWidths: number[] = []
	...
	private setIcePatchesOnGround: ((patches: IcePatchOverlay[]) => void) | null =
		null
	private updateInsetCenter:
		| ((playerX: number, playerZ: number) => void)
		| null = null
	private setViewCenterOnGround:
		| ((x: number, y: number, z: number) => void)
		| null = null
	/** Set the second view center for local multiplayer (both players get high shader detail). */
	private setViewCenter2OnGround:
		| ((x: number, y: number, z: number) => void)
		| null = null
	private setShadowMapOnGround:
		| ((rtt: RenderTargetTexture, cam: FreeCamera, light: SpotLight) => void)
		| null = null
	...
	private onPreviewTick?: (
		elapsed: number,
		finished: number,
		total: number,
	) => void
	private onPreviewCountdown?: (text: string) => void
	private onPreviewRaceState?: (
	...
	constructor(
		...
		options?: {
			...
			p2Role?: 'bus' | 'runner'
			p2CharSelection?: CharacterSelection | null
			p2MinimapCanvas?: HTMLCanvasElement
		},
	) {
		this.canvas = canvas
	...
	startCountdown() {
		...
		const frontDir = new Vector3(
			Math.sin(this.busYaw),
			0,
			Math.cos(this.busYaw),
		)
		const camDist = 18
		const camHeight = 10
		...
		this.camera.setTarget(
			new Vector3(
				this.busPos.x,
				this.busPos.y + (this.localPlayerRole === 'runner' ? 1.4 : 2.5),
				this.busPos.z,
			),
		)
	}
	...
	async buildRemoteBusForPeer(
		peerId: string,
		playerIndex: number,
		remoteCharSel?: CharacterSelection,
	) {
		...
		const remoteIsCorgi =
			...
			isCorgiRunnerId(remoteCharSel.runnerId)
	...
	getLocalPlayerState(): PlayerState {
		return {
			...
			scooped: this.runners.filter(
				(r) =>
					r.state === 'riding' && r.ownerPlayerIndex === this.localPlayerIndex,
			).length,
	...
	private ensureRunnerShoeFlames() {
		if (!this.scene || !this.localRunnerModel || this.shoeFlames.length > 0)
			return
	...
	private updateRunnerPowerUpEffects(dt: number) {
		...
		this.setRunnerShoeFlamesActive(
			this.runnerShoeTimer > 0 && this.localPlayerRole === 'runner',
		)
		...
		if (this.setIcePatchesOnGround) {
			...
			for (const patch of this.icePatches) {
				...
				const fadeStart =
					POWER_UP_ICE_DURATION_SECONDS - POWER_UP_ICE_FADE_SECONDS
				const alpha =
					patch.age > fadeStart
						? POWER_UP_ICE_BASE_ALPHA *
							Math.max(
								0,
								1 - (patch.age - fadeStart) / POWER_UP_ICE_FADE_SECONDS,
							)
						: POWER_UP_ICE_BASE_ALPHA
	...
	private getPlayerPoseByIndex(
		playerIndex: number,
	): { x: number; y: number; z: number; yaw: number; speed: number } | null {
	...
	private beginLocalRunnerScoop(
		_scooperPlayerIndex: number,
		scooperPose: { yaw: number; speed: number },
	) {
		...
		this.localRunnerScoopVelX =
			fwdX * speed * SCOOP_FORWARD_FACTOR + (Math.random() - 0.5) * 3
		this.localRunnerScoopVelY =
			Math.max(SCOOP_MIN_UP, absSpeed * SCOOP_UP_FACTOR) + Math.random() * 3
		this.localRunnerScoopVelZ =
			fwdZ * speed * SCOOP_FORWARD_FACTOR + (Math.random() - 0.5) * 3
	...
	private tryScoopRemoteRunnerPlayersByLocalBus() {
		...
		for (const [_peerId, remote] of this.remotePlayers) {
			...
			if (
				this.getRoleForPlayerIndex(remote.playerIndex, remote.state) !==
				'runner'
			)
				continue
			if (
				this.playerScoopCooldownUntil.get(remote.playerIndex) &&
				nowSec < (this.playerScoopCooldownUntil.get(remote.playerIndex) ?? 0)
			)
				continue
	...
	private updateLocalRunnerScooped(dt: number) {
		if (this.localRunnerScoopState === 'launched') {
			...
			if (
				this.resolvePositionAgainstBuildingsLocal(
					this.busPos,
					RUNNER_COLLISION_RADIUS,
				)
			) {
			...
			if (
				...
				if (speed > 0.001) {
					this.localRunnerScoopVelX =
						(this.localRunnerScoopVelX / speed) * bounceStrength
					this.localRunnerScoopVelZ =
						(this.localRunnerScoopVelZ / speed) * bounceStrength
				}
			}
			...
			if (
				Math.abs(this.localRunnerScoopVelX) > 0.001 ||
				Math.abs(this.localRunnerScoopVelZ) > 0.001
			) {
				this.busYaw = Math.atan2(
					this.localRunnerScoopVelX,
					this.localRunnerScoopVelZ,
				)
			}
	...
	async initPreview(
		...
		this.previewOrbitCleanup = setupPreviewOrbitInput(
			this.canvas,
			this.previewOrbitState,
		)
	...
	private detectMobileLikeDevice(): boolean {
		...
		return (
			/iphone|ipad|ipod|android|mobile|mobi/.test(ua) ||
			coarsePointer ||
			touchPoints
		)
	}
	...
	private configurePerformanceProfile() {
		...
		if (this.mobileLikeDevice) {
			...
			this.effectivePathMaskResolution = Math.min(
				RENDER_PATH_MASK_RESOLUTION,
				2048,
			)
	...
	private async initScene(eventId: string, opts: InitSceneOptions) {
		...
		const course =
			this.useAltCourse && level.altCourse ? level.altCourse : level.course
		...
		const { positions, heights, totalDistance } = gpsToLocal(
			orderedCoords,
			elevations,
		)
		...
		this.scaleFactor =
			totalDistance > 0 ? courseTargetLength / totalDistance : 1
		this.elevationScale = this.scaleFactor * ALTITUDE_EXAGGERATION
		this.originCoord = [course.coordinates[0][0], course.coordinates[0][1]]
		...
		// Parse path type segments and widths from course coordinates
		this.pathTypeSegments = parsePathTypeSegments(orderedCoords)
		this.pathWidths = orderedCoords.map((c) => (c[3] as number) ?? 1)
		...
		this.camera.fov =
			this.localPlayerRole === 'runner' ? CAMERA_FOV_RUNNER : CAMERA_FOV_BUS
		...
		if (this.localMultiplayer) {
			...
			this.p2Camera.fov =
				this.localPlayerRole === 'runner' ? CAMERA_FOV_BUS : CAMERA_FOV_RUNNER
		...
		this.waterZones = computeWaterZones(
			waterFeatures,
			this.originCoord,
			this.scaleFactor,
			(x, z) => this.getTerrainHeight(x, z),
		)
		this.roadPolylines = computeRoadPolylines(
			roadFeatures,
			this.originCoord,
			this.scaleFactor,
		)
		this.trailPolylines = pathFeatures
			.map((p) =>
			...
			.filter((t) => t.length >= 2)
		this.buildingFootprints = computeBuildingFootprintData(
			buildingFeatures,
			this.originCoord,
			this.scaleFactor,
		)
		...
		this.fieldPolygons = regionEntries
			.filter((r) => r.type === 'field')
			.map((r) => r.points)
		this.concretePolygons = regionEntries
			.filter((r) => r.type === 'concrete')
			.map((r) => r.points)
		this.regionEntries = regionEntries
		this.buildGround()
		buildWaterMeshes(this.scene, this.waterZones, { isNight })
		const buildingResult = buildBuildingMeshes(
			this.scene,
		...
		const bridgeWorldData = bridgeFeatures.map((b) => {
			const [rawSx, rawSz] = gpsPointToLocal(
				b.points[0][1],
				b.points[0][0],
				this.originCoord,
			)
			const [rawEx, rawEz] = gpsPointToLocal(
				b.points[1][1],
				b.points[1][0],
				this.originCoord,
			)
			return {
				start: [rawSx * this.scaleFactor, rawSz * this.scaleFactor] as [
					number,
					number,
				],
				end: [rawEx * this.scaleFactor, rawEz * this.scaleFactor] as [
					number,
					number,
				],
			}
		})
		const bridgeResult = buildBridgeMeshes(
			this.scene,
			bridgeWorldData,
			(x, z) => this.getGroundY(x, z),
		)
		this.bridgeColliders = bridgeResult.colliders
		this.bridgeEndpoints = bridgeWorldData.map((b) => ({
			start: b.start,
			end: b.end,
		}))
		if (level.trees !== false) {
			const manualTreePositions: [number, number][] = (
				level.manualTrees ?? []
		...
		// Cones along both sides of grass course segments (every 3m)
		{
			const coneResult = placeGrassPathCones(
				this.scene,
				this.pathPositions,
				this.pathTypeSegments,
				this.pathWidths,
				(x, z) => this.getGroundY(x, z),
			)
			const coneElasticOffset = this.elasticObjects.length
			for (const obs of coneResult.solidObstacles) {
				if (obs.elasticIndex != null) obs.elasticIndex += coneElasticOffset
			}
			this.elasticObjects.push(...coneResult.elasticObjects)
			this.solidObstacles.push(...coneResult.solidObstacles)
			this.objectRoots.push(...coneResult.coneRoots)
		}
		...
		// ── Geese (scoopable AI entities) ──
		{
			...
			if (geeseData.length > 0) {
				this.geese = spawnGeese(this.scene, geeseData, (x, z) =>
					this.getGroundY(x, z),
				)
			}
		}

		// ── Swans (scoopable AI entities, same behaviour as geese) ──
		{
			const swanData = (level.objects?.swans ?? []).map(([lat, lon, rot]) => {
				const [rawX, rawZ] = gpsPointToLocal(lon, lat, this.originCoord)
				return {
					x: rawX * this.scaleFactor,
					z: rawZ * this.scaleFactor,
			...
			console.log('[Swans] spawn data:', swanData.length, 'swans')
			if (swanData.length > 0) {
				this.swans = spawnSwans(this.scene, swanData, (x, z) =>
					this.getGroundY(x, z),
				)
			}
		}
		...
		// ── Deer (scoopable AI entities, faster with larger herds) ──
		{
			...
			if (deerData.length > 0) {
				this.deer = spawnDeer(this.scene, deerData, (x, z) =>
					this.getGroundY(x, z),
				)
			}
		}
		...
		if (!this.demoMode && this.localPlayerRole === 'runner') {
			const localIsCorgi =
				...
				isCorgiRunnerId(this.charSelection.runnerId)
			const runnerAppearance =
				!localIsCorgi && this.charSelection?.type === 'runner'
					? resolveRunnerAppearance(this.charSelection.runnerId)
					: undefined
			this.localRunnerModel = buildLocalRunnerFn(
				this.scene,
		...
		if (this.localMultiplayer) {
			...
			await this.buildRemoteBusForPeer(
				'__p2__',
				2,
				this.p2CharSelection ?? undefined,
			)
			if (this._disposed) return
		...
		if (!opts.skipRunners && this.gameType !== 'single-bus-mode') {
			this.runners = spawnRunnersSystem(
				this.scene,
				this.pathPositions,
				(x, z) => this.getGroundY(x, z),
			)
			...
			this.passengerSystem = new PassengerSystem(
				...
				(x, z) => this.getGroundY(x, z),
				{
					...
					onTimerChange: (remaining) =>
						this.callbacks.onBusModeTimer?.(remaining),
		...
		if (level.kmSigns !== false) {
			const kmResult = placeKmSigns(this.scene, this.pathPositions, (x, z) =>
				this.getGroundY(x, z),
			)
		...
		this.gatePositions = buildGatesSystem(
			this.pathPositions,
			this.pathCumDist,
			(x, z) => this.getGroundY(x, z),
		)
		...
		this.powerUpSystem = new PowerUpSystem({
			...
			onPowerUpDisplayChange: (powerUp, rolling) =>
				this.callbacks.onPowerUpDisplayChange?.(powerUp, rolling),
		...
		if (this.pathPositions.length > 1) {
			...
			if (level.parkrunSign !== false) {
				const startLineResult = buildStartLineObjects(
					this.scene,
					this.pathPositions,
					eventId,
					(x, z) => this.getGroundY(x, z),
			...
			this.busPos.x =
				sx - forwardX * BUS_START_OFFSET + rightX * laneOffset * lateralSpacing
			this.busPos.z =
				sz - forwardZ * BUS_START_OFFSET + rightZ * laneOffset * lateralSpacing
		...
		if (level.showMarshals !== false) {
			const marshalResult = spawnMarshalsSystem(
				this.scene,
				level,
				this.pathPositions,
				this.originCoord,
		...
		const landmarkResult = placeEventLandmarksSystem(
			this.scene,
			eventId,
			this.originCoord,
			this.scaleFactor,
		...
		if (level.fences !== false) {
			const fenceDistance = level.fenceDistance ?? DEFAULT_FENCE_DISTANCE
			const boundaryPoly = generateFencePolygon(
				this.pathPositions,
				fenceDistance,
			)
			if (boundaryPoly.length >= 3) {
				this.fenceCollider = buildFenceMesh(this.scene, boundaryPoly, (x, z) =>
					this.getGroundY(x, z),
				)
			}
		}
		...
		if (level.customFences) {
			for (const fence of level.customFences) {
				const localPoly: [number, number][] = fence.points.map(([lat, lon]) => {
					const [rawX, rawZ] = gpsPointToLocal(lon, lat, this.originCoord)
					return [rawX * this.scaleFactor, rawZ * this.scaleFactor] as [
						number,
						number,
					]
				})
				const customCollider = buildFenceMesh(this.scene, localPoly, (x, z) =>
					this.getGroundY(x, z),
				)
	...
	private buildGround() {
		...
		const shaderOpts = {
			...
			pathTypeSegments: this.pathTypeSegments,
		...
		if (positions) {
			for (let i = 0; i < positions.length; i += 3) {
				...
				const waterInfo = getWaterDepressionAt(
					x,
					z,
					this.waterZones,
	...
	private isOnDirtPath(x: number, z: number): boolean {
		...
		const onMainPath =
			!this.pathTextureUrl &&
			distToPath(x, z, this.pathPositions) < PATH_HALF_WIDTH
	...
	private resolvePositionAgainstBuildingsLocal(
		pos: Vector3,
		radius: number,
	): boolean {
	...
	private buildGateFlag() {
		...
		const pole = MeshBuilder.CreateCylinder(
			'gateFlagPole',
			{ diameter: 0.14, height: poleHeight, tessellation: 8 },
			this.scene,
		)
		...
		const banner = MeshBuilder.CreateGround(
			'gateFlagBanner',
			{ width: bannerW, height: bannerH, subdivisions: 10, updatable: true },
			this.scene,
		)
		...
		this.scene.registerBeforeRender(() => {
			...
			for (let i = 0; i < positions.length; i += 3) {
				...
				positions[i + 1] =
					restPositions[i + 1] + Math.sin(t * 8 + nx * 6) * amplitude
			}
	...
	private buildP2GateFlag() {
		...
		this.scene.registerBeforeRender(() => {
			...
			for (let i = 0; i < positions.length; i += 3) {
				...
				const amplitude = nx * nx * 1.2
				positions[i + 1] =
					restPositions2[i + 1] + Math.sin(t * 8 + nx * 6 + 1) * amplitude
			}
			banner.updateVerticesData(VertexBuffer.PositionKind, positions)
		})
	...
	private buildFinishFlag() {
		...
		this.scene.registerBeforeRender(() => {
			...
			for (let i = 0; i < positions.length; i += 3) {
				...
				const amplitude = nx * nx * 1.2
				positions[i + 1] =
					finishRestPositions[i + 1] + Math.sin(t * 8 + nx * 6 + 2) * amplitude
			}
			banner.updateVerticesData(VertexBuffer.PositionKind, positions)
		})
	...
	private async buildDirectionArrowForCamera(
		camera: FreeCamera,
		suffix: string,
	) {
		// ── Node hierarchy ──
	...
	private updateDirectionArrowForPlayer(
		...
		if (this.passengerSystem) {
			target = this.passengerSystem.getClosestFlagTarget(
				playerPos.x,
				playerPos.z,
			)
			if (!target) {
				target = this.passengerSystem.getClosestWaitingPassenger(
					playerPos.x,
					playerPos.z,
				)
			}
		...
		let displayAngle =
			_suffix === 'p2' ? this.p2ArrowDisplayAngle : this.arrowDisplayAngle
	...
	private async buildBus() {
		...
		if (this.level?.timeOfDay === 'night') {
			const headlight = new SpotLight(
				'busHeadlight',
				new Vector3(
					LIGHT_FRONT_POSITION[0],
					LIGHT_FRONT_POSITION[1],
					LIGHT_FRONT_POSITION[2],
				),
			...
			const shadowRTT = new RenderTargetTexture(
				'shadowMap',
				1024,
				this.scene,
				false,
	...
	handleRemoteScoop(
		...
		runner.velX = scooperSpeed
			? Math.sin(scooperYaw ?? 0) * scooperSpeed * SCOOP_FORWARD_FACTOR
			: 0
		runner.velZ = scooperSpeed
			? Math.cos(scooperYaw ?? 0) * scooperSpeed * SCOOP_FORWARD_FACTOR
			: 0
		runner.velY = scooperSpeed
			? scooperSpeed * SCOOP_UP_FACTOR
			: SCOOP_UP_FACTOR
	}
	...
	private setupInput() {
		...
		const onTouchMove = (e: TouchEvent) => {
			...
			this.touchDeltaX =
				Math.abs(rawX) < DEAD_ZONE
					? 0
					: Math.max(
							-1,
							Math.min(
								1,
								(rawX - Math.sign(rawX) * DEAD_ZONE) / (MAX_DRAG - DEAD_ZONE),
							),
						)
			this.touchDeltaY =
				Math.abs(rawY) < DEAD_ZONE
					? 0
					: Math.max(
							-1,
							Math.min(
								1,
								(rawY - Math.sign(rawY) * DEAD_ZONE) / (MAX_DRAG - DEAD_ZONE),
							),
						)
		}
				...
				if (
					...
					if (scoopedRoot) {
						this.scoopedObjects.push({
							root: scoopedRoot,
							velX:
								fwdX * this.busSpeed * SCOOP_FORWARD_FACTOR +
								(Math.random() - 0.5) * 3,
							velY:
								Math.max(SCOOP_MIN_UP, absSpeed * SCOOP_UP_FACTOR) +
								Math.random() * 3,
							velZ:
	...
	private resolveBusCollisions() {
		...
		for (const [peerId, remote] of this.remotePlayers) {
			if (!remote.state) continue
			if (
				this.getRoleForPlayerIndex(remote.playerIndex, remote.state) !== 'bus'
			)
				continue
			remotes.push({
				id: peerId,
		...
		const { local, collided, remoteNudges } = resolveBusToBusCollisions(
			localState,
			remotes,
			0,
		)
	...
	private resolveRemotePlayerElasticCollisions() {
		for (const [_peerId, remote] of this.remotePlayers) {
			...
			const remoteRole = this.getRoleForPlayerIndex(
				remote.playerIndex,
				remote.state,
			)
			const collisionRadius =
				remoteRole === 'runner' ? RUNNER_COLLISION_RADIUS : BUS_COLLISION_RADIUS
	...
	private tryScoopNpcRunnersForP2() {
		...
		for (const runner of this.runners) {
			...
			const fwdZ = Math.cos(this.p2Yaw)
			runner.velX =
				fwdX * this.p2Speed * SCOOP_FORWARD_FACTOR + (Math.random() - 0.5) * 3
			runner.velY =
				Math.max(SCOOP_MIN_UP, absSpd * SCOOP_UP_FACTOR) + Math.random() * 3
			runner.velZ =
				fwdZ * this.p2Speed * SCOOP_FORWARD_FACTOR + (Math.random() - 0.5) * 3
	...
	private updateP2(dt: number) {
		...
		const enterPressed =
			(enterHeld && !this.p2EnterHeldLast) || gp2.actionPressed
		...
		if (Math.abs(gp2.cameraX) > 0) {
			...
			if (Math.abs(this.gamepadCameraOrbitP2) < 0.01)
				this.gamepadCameraOrbitP2 = 0
		}
		...
		if (!this.p2Finished) {
			if (this.p2Role === 'runner') {
				...
				if (turnInput !== 0)
					this.p2Yaw += turnInput * RUNNER_PLAYER_TURN_SPEED * dt
				...
				const blend = Math.min(
					...
					(accelInput !== 0
						? RUNNER_PLAYER_ACCELERATION
						: RUNNER_PLAYER_DECELERATION) * dt,
				)
				this.p2Speed += (targetSpeed - this.p2Speed) * blend
				this.p2Speed = Math.max(
					-RUNNER_PLAYER_SPEED * REVERSE_SPEED_MULTIPLIER,
					Math.min(RUNNER_PLAYER_SPEED, this.p2Speed),
				)
				...
				if (this.p2BoostTimer > 0) {
					...
					this.p2Speed = Math.min(
						boostedMax,
						this.p2Speed + SCOOP_BOOST_ACCELERATION * dt,
					)
				...
				if (this.p2BoostTimer > 0) {
					...
					effectiveMaxSpeed =
						BUS_MAX_SPEED + BUS_MAX_SPEED * (SCOOP_BOOST_MULTIPLIER - 1) * t
				} else {
					effectiveMaxSpeed = BUS_MAX_SPEED
				}
				this.p2Speed = Math.max(
					-effectiveMaxSpeed * REVERSE_SPEED_MULTIPLIER,
					Math.min(effectiveMaxSpeed, this.p2Speed),
				)
				...
				const forward = new Vector3(
					Math.sin(this.p2VelAngle),
					0,
					Math.cos(this.p2VelAngle),
				)
				...
				if (slopePitch > 0.01 && this.p2Speed > 0 && this.p2BoostTimer <= 0) {
					...
					this.p2Speed = Math.max(
						0,
						this.p2Speed - BUS_UPHILL_DRAG * uphillFactor * dt,
					)
				}
			...
			if (this.p2Speed > 0) this.p2Speed = Math.max(0, this.p2Speed - 8 * dt)
				...
				this.p2Speed = Math.min(0, this.p2Speed + 8 * dt)
		...
		// ── Gate check for P2 ──
		const newP2GateIdx = checkGatePass(
			this.gatePositions,
			this.p2GateIdx,
			this.p2Pos.x,
			this.p2Pos.z,
		...
		if (
			!this.p2Finished &&
			this.gameType !== 'single-bus-mode' &&
			this.p2GateIdx >= this.gatePositions.length &&
		...
		if (this.raceState === 'racing') {
			...
			const p2CoveredKm =
				(this.p2GateIdx > 0
					...
					: 0) / 1000
			this.callbacks.onP2CourseProgress?.(
				Number.parseFloat(p2CoveredKm.toFixed(1)),
				Number.parseFloat(totalKm.toFixed(1)),
			)
		}
	...
	private updateP2Camera(dt: number) {
		...
		const gpOrbitP2 =
			this.gamepadCameraOrbitP2 + (this.gamepadRearCamP2 ? Math.PI : 0)
		...
		if (this.p2Role === 'runner') {
			...
			this.p2CameraYawOffset +=
				Math.sign(yawDiff) * Math.min(Math.abs(yawDiff), camSwingSpeed * dt)
		...
		const p2BoostIntensity =
			Math.abs(this.p2Speed) > BUS_MAX_SPEED
				? Math.min(
						1,
						(Math.abs(this.p2Speed) - BUS_MAX_SPEED) /
							(BUS_MAX_SPEED * (SCOOP_BOOST_MULTIPLIER - 1) * 0.5),
					)
	...
	private updateChaseCam(dt: number) {
		...
		const gpOrbitP1 =
			this.gamepadCameraOrbitP1 + (this.gamepadRearCamP1 ? Math.PI : 0)
	...
	private getMinimapLookaheadAnchor(): {
		x: number
		z: number
		pathDist: number
		...
		return {
			...
			z: this.busPos.z,
			pathDist: totalDist,
			playerPathDist: totalDist,
		}
	}

	private getP2MinimapLookaheadAnchor(): {
		x: number
		z: number
		pathDist: number
		playerPathDist: number
	...
	private update(dt: number) {
		...
		if (this.localMultiplayer && this.p2Camera) {
			...
			this.setViewCenter2OnGround?.(
				p2View.x,
				this.p2Camera.position.y,
				p2View.z,
			)
		...
		const engineVibeOffset =
			Math.sin(this.engineVibePhase) * ENGINE_VIBE_AMPLITUDE * speedFraction
		this.powerUpSystem?.update(Date.now())
		this.updateRunnerPowerUpEffects(dt)
		this.updateLocalReverseLights()

		const enterHeld =
			this.isKey('enter') || this.gamepadManager.getState(0).itemPressed
		if (
			(enterHeld && !this.enterHeldLastFrame) ||
			this.gamepadManager.getState(0).itemPressed
		) {
			this.useHeldPowerUp()
		}
		...
		if (Math.abs(gp1.cameraX) > 0) {
			...
			if (Math.abs(this.gamepadCameraOrbitP1) < 0.01)
		...
		if (this.raceState === 'countdown') {
			...
			if (runnerResult.boostTimerAdd > 0)
				this.boostTimer += runnerResult.boostTimerAdd
			if (
				runnerResult.startExhaust &&
				this.exhaustFlames &&
				!this.exhaustFlames.isStarted()
			) {
			...
			// Update swans during countdown
			if (this.swans.length > 0) {
				updateSwansSystem(
					{
						scene: this.scene,
						swans: this.swans,
				...
				this.localRunnerAnimPhase = updateLocalRunnerVisualFn(
					{
						...
						busSpeed:
							this.busSpeed *
							(this.runnerFikaTimer > 0
								? POWER_UP_FIKA_ANIM_SPEED_MULTIPLIER
								: 1),
						busAirborne: this.busAirborne,
						scoopState: this.localRunnerScoopState,
						runnerJumpSideVel: this.runnerJumpSideVel,
						jumpsUsed: this.runnerJumpsUsed,
						flipDirection: this.runnerFlipDirection,
						getGroundY: (x, z) => this.getGroundY(x, z),
						inWater:
							this.getWaterSurfaceY(this.busPos.x, this.busPos.z) !== null,
					},
					dt,
					this.localRunnerAnimPhase,
				)
			}
			...
			const totalKm = this.pathTotalDistance / 1000
			this.callbacks.onCourseProgress?.(
				0,
				Number.parseFloat(totalKm.toFixed(1)),
			)
			...
			// --- Engine sounds during countdown (with rev on forward input) ---
			{
				...
				const revving =
					this.isKey('w') ||
					(!this.localMultiplayer && this.isKey('arrowup')) ||
					(this.touchActive && this.touchDeltaY <= 0.1) ||
					gp1State.accel > 0.1
				const revSpeed = revving ? 8 : 0 // simulate ~8 m/s for rev sound
				const busSources: BusSoundSource[] = []
				// Only emit engine sound for local player if they are a bus
				if (this.localPlayerRole === 'bus') {
					busSources.push({
						x: this.busPos.x,
						y: this.busPos.y,
						z: this.busPos.z,
				...
				for (const [, remote] of this.remotePlayers) {
					if (remote.state) {
						busSources.push({
							x: remote.smoothPos.x,
							y: remote.smoothPos.y,
							z: remote.smoothPos.z,
							speed: remote.state.speed,
							slope: remote.smoothPitch,
						})
					}
				}
				const goosePositions = [
					...this.geese.map((g) => ({ x: g.x, z: g.z })),
					...this.swans.map((s) => ({ x: s.x, z: s.z })),
				]
		...
		if (this.raceState === 'finished') {
			...
			if (this.keepDrivingMode) {
				...
				if (finWaterY !== null) {
					...
					finGroundY =
						finWaterY +
						Math.sin(this.waterBobPhase) * WATER_BOB_AMPLITUDE -
						WATER_SINK
				}
				this.waterWakeActive = setWaterWakeActive(
					this.waterWake,
					this.waterWakeActive,
					finWaterY !== null && Math.abs(this.busSpeed) > 0.5,
				)
				...
				const finOnDirt =
					finWaterY === null &&
					Math.abs(this.busSpeed) > 1 &&
					this.isOnDirtPath(this.busPos.x, this.busPos.z)
				this.dirtSprayActive = setDirtSprayActive(
					this.dirtSpray,
					this.dirtSprayActive,
					finOnDirt,
				)
				updateDirtSprayIntensity(
					this.dirtSpray,
				...
				this.boostEffects?.update(
					dt,
					finishBoostIntensity,
					this.boostEffectsElapsed,
				)
				...
				if (runnerResult2.boostTimerAdd > 0)
					this.boostTimer += runnerResult2.boostTimerAdd
				if (
					runnerResult2.startExhaust &&
					this.exhaustFlames &&
					!this.exhaustFlames.isStarted()
				) {
					this.exhaustFlames.start()
		...
		const totalKm = this.pathTotalDistance / 1000
		const coveredDist =
			this.currentGateIdx < this.gatePositions.length
		...
		const coveredKm =
			(this.currentGateIdx > 0
				...
				: 0) / 1000
		this.callbacks.onCourseProgress?.(
			Number.parseFloat(coveredKm.toFixed(1)),
			Number.parseFloat(totalKm.toFixed(1)),
		)
		...
		let accelInput = 0
		if (this.isKey('w') || (!this.localMultiplayer && this.isKey('arrowup')))
			accelInput += 1
		if (this.isKey('s') || (!this.localMultiplayer && this.isKey('arrowdown')))
			accelInput -= 1
		...
		// --- Steering ---
		let turnInput = 0
		if (this.isKey('a') || (!this.localMultiplayer && this.isKey('arrowleft')))
			turnInput -= 1
		if (this.isKey('d') || (!this.localMultiplayer && this.isKey('arrowright')))
			turnInput += 1
		...
		if (this.localPlayerRole === 'runner') {
			if (this.localRunnerScoopState !== 'free') {
				...
				const playerRunnerHeading = new Vector3(
					Math.sin(this.busYaw),
					0,
					Math.cos(this.busYaw),
				)
				const currentRunnerHeight = this.getGroundY(
					this.busPos.x,
					this.busPos.z,
				)
				...
				const playerRunnerSlope = Math.atan2(
					aheadRunnerHeight - currentRunnerHeight,
					2,
				)
				const playerSpeedMultiplier =
					playerRunnerSlope < RUNNER_DOWNHILL_SLOPE_THRESHOLD
						? RUNNER_DOWNHILL_SPEED_BOOST
						: 1

				const fikaSpeedBoost =
					this.runnerFikaTimer > 0 ? POWER_UP_FIKA_SPEED_MULTIPLIER : 1
				const shoeSpeedBoost =
					this.runnerShoeTimer > 0 ? POWER_UP_SHOE_SPEED_MULTIPLIER : 1
				...
				const maxReverse =
					this.runnerShoeTimer > 0
						? 0
						: RUNNER_PLAYER_SPEED * REVERSE_SPEED_MULTIPLIER
				this.busSpeed = Math.max(
					-maxReverse,
					Math.min(
						RUNNER_PLAYER_SPEED * playerSpeedMultiplier * totalRunnerSpeedBoost,
						this.busSpeed,
					),
				)
				...
				const jumpHeld =
					...
					gp1.actionHeld
				const jumpPressed =
					(jumpHeld && !this.jumpHeldLastFrame) || gp1.actionPressed
				...
				if (jumpPressed && this.runnerJumpsUsed < RUNNER_PLAYER_MAX_JUMPS) {
					...
					const leftHeld =
						this.isKey('a') ||
						(!this.localMultiplayer && this.isKey('arrowleft'))
					const rightHeld =
						this.isKey('d') ||
						(!this.localMultiplayer && this.isKey('arrowright'))
					const backHeld =
						this.isKey('s') ||
						(!this.localMultiplayer && this.isKey('arrowdown'))
					if (leftHeld !== rightHeld) {
						this.runnerJumpSideVel = leftHeld
							? -RUNNER_PLAYER_JUMP_SIDE_VELOCITY
							: RUNNER_PLAYER_JUMP_SIDE_VELOCITY
					} else {
						this.runnerJumpSideVel = 0
				...
				this.busPos.x +=
					(forward.x * this.busSpeed + right.x * this.runnerJumpSideVel) * dt
				this.busPos.z +=
					(forward.z * this.busSpeed + right.z * this.runnerJumpSideVel) * dt
				...
				// If over water, float on the surface with a gentle bob, half submerged
				const runnerWaterSurfY = this.getWaterSurfaceY(
					this.busPos.x,
					this.busPos.z,
				)
				if (runnerWaterSurfY !== null) {
					this.waterBobPhase += WATER_BOB_SPEED * dt
					const bob = Math.sin(this.waterBobPhase) * WATER_BOB_AMPLITUDE
					groundY = runnerWaterSurfY + bob - RUNNER_WATER_SINK
				}
				this.waterWakeActive = setWaterWakeActive(
					this.waterWake,
					this.waterWakeActive,
					runnerWaterSurfY !== null && !this.busAirborne,
				)
				updateWakeIntensity(this.waterWake, this.waterWakeActive, this.busSpeed)
				this.dirtSprayActive = setDirtSprayActive(
					this.dirtSpray,
			...
			if (this.frontWheelLeft) {
				this.frontWheelLeft.rotation.y +=
					(targetWheelY - this.frontWheelLeft.rotation.y) * steerSmooth
			}
			if (this.frontWheelRight) {
				this.frontWheelRight.rotation.y +=
					(targetWheelY - this.frontWheelRight.rotation.y) * steerSmooth
			}
			...
			for (const pivot of allWheelPivots) {
				const wheelRoot = pivot
					.getChildren()
					.find((c) => c.name.startsWith('wheelGlbRoot'))
				if (wheelRoot) {
					// Right-side wheels are flipped (rotation.y = PI) so negate roll direction
					const isRight =
						pivot.name.includes('Right') || pivot.name === 'rearWheelPivot_2'
			...
			if (this.boostTimer > 0) {
				// Boosting: rapidly accelerate to boosted max speed, cannot slow down
				const boostedMax = BUS_MAX_SPEED * SCOOP_BOOST_MULTIPLIER
				this.busSpeed = Math.min(
					boostedMax,
					this.busSpeed + SCOOP_BOOST_ACCELERATION * dt,
				)
			} else if (accelInput > 0) {
			...
			this.busSpeed = Math.max(
				-effectiveMaxSpeed * REVERSE_SPEED_MULTIPLIER,
				Math.min(effectiveMaxSpeed, this.busSpeed),
			)
			...
			const forward = new Vector3(
				Math.sin(this.busVelAngle),
				0,
				Math.cos(this.busVelAngle),
			)
			...
			this.waterWakeActive = setWaterWakeActive(
				this.waterWake,
				this.waterWakeActive,
				waterSurfY !== null && !this.busAirborne,
			)
			updateWakeIntensity(this.waterWake, this.waterWakeActive, this.busSpeed)
			...
			if (
				...
				this.busSpeed = Math.max(
					0,
					this.busSpeed - BUS_UPHILL_DRAG * uphillFactor * dt,
				)
			}
			...
			if (this.busAirborne) {
				...
				const flightPitch =
					speedXZ > 0.5 ? Math.atan2(this.busVelY, speedXZ) : 0
				this.busPitch +=
					(flightPitch - this.busPitch) *
					Math.min(1, BUS_AIRBORNE_PITCH_LERP * dt)
				...
				if (!launched) {
					...
					if (
						pitchDrop > BUS_JUMP_PITCH_THRESHOLD &&
						absSpeed > 4 &&
						this.prevSlopePitch > 0.03
					) {
		...
		const boostIntensity =
			Math.abs(this.busSpeed) > BUS_MAX_SPEED
				? Math.min(
						1,
						(Math.abs(this.busSpeed) - BUS_MAX_SPEED) /
							(BUS_MAX_SPEED * (SCOOP_BOOST_MULTIPLIER - 1) * 0.5),
		...
		const newGateIdx = checkGatePass(
			this.gatePositions,
			this.currentGateIdx,
			this.busPos.x,
			this.busPos.z,
		...
		if (newGateIdx > this.currentGateIdx) {
			...
			this.callbacks.onGatePass?.(
				this.currentGateIdx,
				this.gatePositions.length,
			)
			this.updateGateFlag()
		}
		...
		if (runnerResult3.boostTimerAdd > 0)
			this.boostTimer += runnerResult3.boostTimerAdd
		if (
			runnerResult3.startExhaust &&
			this.exhaustFlames &&
			!this.exhaustFlames.isStarted()
		) {
			this.exhaustFlames.start()
		...
		// Update runner social interactions (wave / high-five)
		{
			...
			if (
				isRunner &&
				this.localRunnerModel &&
				this.playerInteraction.interaction !== 'none'
			) {
				const pi = this.playerInteraction
				const totalDur =
					pi.interaction === 'highfive' ? HIGH_FIVE_DURATION : WAVE_DURATION
		...
		if (this.passengerSystem && this.raceState === 'racing') {
			const pResult = this.passengerSystem.update(
				dt,
				this.busPos,
				this.busYaw,
				this.busSpeed,
