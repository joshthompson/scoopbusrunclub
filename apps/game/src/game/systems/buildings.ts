...
function getOrCreateFacadeTemplates(
	...
	const innerWidth = Math.max(
		outerWidth - frameThickness * 2,
		outerWidth * 0.45,
	)
	const innerHeight = Math.max(
		outerHeight - frameThickness * 2,
		outerHeight * 0.45,
	)
	...
	const frameV = MeshBuilder.CreateBox(
		`tpl_fv_${key}`,
	...
	const frameH = MeshBuilder.CreateBox(
		`tpl_fh_${key}`,
	...
	if (addMullions) {
		...
		mullionV = MeshBuilder.CreateBox(
			`tpl_mv_${key}`,
		...
		mullionH = MeshBuilder.CreateBox(
			`tpl_mh_${key}`,
...
export function buildBuildingMeshes(
	...
	const brickWallMat = new StandardMaterial('buildingBrickMat', scene)
	brickWallMat.diffuseColor = new Color3(0.72, 0.32, 0.18)
	brickWallMat.specularColor = Color3.Black()

	const brickRoofMat = new StandardMaterial('buildingBrickRoofMat', scene)
	brickRoofMat.diffuseColor = new Color3(0.42, 0.28, 0.22)
	brickRoofMat.specularColor = Color3.Black()
	...
	greenGrooveMat.diffuseColor = new Color3(0.42, 0.5, 0.36)
	...
	for (let i = 0; i < footprints.length; i++) {
		...
		let minFwd = Number.POSITIVE_INFINITY
		let maxFwd = Number.NEGATIVE_INFINITY
		...
		const isSwedishHouse =
			building.type === 'red' ||
			building.type === 'green' ||
			building.type === 'yellow'
		const isBrickHouse = building.type === 'brick'
		const trimMat = isSwedishHouse
			? trimWhiteMat
			: isBrickHouse
				? trimWhiteMat
				: greyTrimMat
				...
				: building.type === 'green'
					...
					: building.type === 'yellow'
						...
						: building.type === 'brick'
							? brickWallMat
							: stoneWallMat
		const activeRoofMat = isBrickHouse ? brickRoofMat : roofMat

		const centerOffsetRight = (minRight + maxRight) * 0.5
		const centerOffsetFwd = (minFwd + maxFwd) * 0.5
		const centerX =
			centroidX + rightX * centerOffsetRight + fwdX * centerOffsetFwd
		const centerZ =
			centroidZ + rightZ * centerOffsetRight + fwdZ * centerOffsetFwd
		...
		lodEntries.push({
			...
			z: centerZ,
			swapDistanceSq:
				RENDER_BUILDING_LOD_DISTANCE * RENDER_BUILDING_LOD_DISTANCE,
		...
		try {
			const foundPts = localPolyPts.map(
				(v) => new Vector3(v.x * 1.02, 0, v.z * 1.02),
			)
		...
		if (isSwedishHouse) {
			const activeGrooveMat =
				building.type === 'green'
					? greenGrooveMat
					: building.type === 'yellow'
						? yellowGrooveMat
						: grooveMat
			addSwedishHouseTrimPolygon(
				scene,
				root,
				i,
				localPolyPts,
...
function addBuildingRoof(
	...
	const slopeLength =
		Math.sqrt(roofSpan * roofSpan + roofHeight * roofHeight) + roofOverhang
	...
	for (const side of [-1, 1]) {
		...
		fascia.position.set(
			0,
			-roofThickness * 0.36,
			side > 0 ? roofDepth * 0.5 : -roofDepth * 0.5,
		)
...
function createGableTriangle(
	...
	const indices = z > 0 ? [0, 1, 2, 5, 4, 3] : [1, 0, 2, 3, 4, 5]
...
function createFramedFacadeRect(
	...
	const tpl = getOrCreateFacadeTemplates(
		facadeCache,
		scene,
		outerWidth,
		outerHeight,
...
function addWindowsOnWall(
	...
	const windowOuterW = Math.max(
		0.9,
		Math.min(1.5, usableSpan / Math.max(2.8, count + 1)),
	)
...
function addBuildingFacadeFeaturesPolygon(
	...
	for (let i = 0; i < wallSegments.length; i++) {
		if (wallSegments[i].span > maxSpan) {
			maxSpan = wallSegments[i].span
			frontIdx = i
		}
	}
	...
	const rows =
		wallHeight > 5.2
			? [
					foundationHeight + wallHeight * 0.38,
					foundationHeight + wallHeight * 0.72,
				]
...
function addWindowsOnFace(
	...
	const count =
		facadeSpan >= 15 ? 4 : facadeSpan >= 9 ? 3 : facadeSpan >= 5.5 ? 2 : 1
...
export function addBuildingFacadeFeatures(
	...
	const sideRows =
		wallHeight > 5.2
			? [
					foundationHeight + wallHeight * 0.42,
					foundationHeight + wallHeight * 0.72,
				]
	...
	addWindowsOnFace(
		scene,
		root,
		`buildingFrontWindows_${buildingIndex}`,
		width,
		depth * 0.5 + facadeOffset,
		frontBackRows,
		0,
	...
	addWindowsOnFace(
		scene,
		root,
		`buildingBackWindows_${buildingIndex}`,
		width,
		-(depth * 0.5 + facadeOffset),
		frontBackRows,
		Math.PI,
	...
	addWindowsOnFace(
		scene,
		root,
		`buildingLeftWindows_${buildingIndex}`,
		depth,
		-(width * 0.5 + facadeOffset),
		sideRows,
		-Math.PI * 0.5,
		false,
		0,
	...
	addWindowsOnFace(
		scene,
		root,
		`buildingRightWindows_${buildingIndex}`,
		depth,
		width * 0.5 + facadeOffset,
		sideRows,
		Math.PI * 0.5,
...
function addSwedishHouseTrimPolygon(
	...
	chimney.position.set(
		width * 0.12,
		foundationHeight + wallHeight + Math.max(1.35, wallHeight * 0.45),
		-depth * 0.12,
	)
	...
	chimneyCap.position.set(
		chimney.position.x,
		chimney.position.y + Math.max(1.1, wallHeight * 0.25) * 0.5 + 0.1,
		chimney.position.z,
	)
...
export function addSwedishHouseTrim(
	...
	for (const sx of [-1, 1]) {
		for (const sz of [-1, 1]) {
			...
			corner.position.set(
				sx * (width * 0.5 - board * 0.25),
				topY,
				sz * (depth * 0.5 + boardDepth * 0.5),
			)
			corner.material = trimMat
			corner.parent = root
			...
			sideBoard.position.set(
				sx * (width * 0.5 + boardDepth * 0.5),
				topY,
				sz * (depth * 0.5 - board * 0.25),
			)
	...
	chimneyCap.position.set(
		...
		chimney.position.y +
			chimney.scaling.y * 0 +
			Math.max(1.1, wallHeight * 0.25) * 0.5 +
			0.1,
		chimney.position.z,
	)
	chimneyCap.material = roofMat
...
export function resolveBuildingCollisionAt(
	...
	const clampedRight = Math.max(
		-collider.halfWidth,
		Math.min(collider.halfWidth, localRight),
	)
	const clampedFwd = Math.max(
		-collider.halfDepth,
		Math.min(collider.halfDepth, localFwd),
	)
...
export function resolvePositionAgainstBuildings(
	...
	for (let pass = 0; pass < 3; pass++) {
		...
		for (const collider of colliders) {
			const resolved = resolveBuildingCollisionAt(
				pos.x,
				pos.z,
				radius,
				collider,
