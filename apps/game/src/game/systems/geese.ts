...
function updateHerds(geese: Goose[]) {
  ...
  const active = geese.filter(g => g.state !== 'launched' && g.state !== 'landed');
...
function getHerdTarget(geese: Goose[], herdId: number): { tx: number; tz: number } | null {
    ...
    switch (goose.state) {
        ...
        if (goose.idleTimer <= 0) {
          ...
          if (goose.herdId !== -1) {
            ...
            if (herdTarget) {
              ...
              target = pickWanderTarget(goose, ctx.getGroundY, ctx.waterZones, ctx.solidObstacles, _rand);
            }
          } else {
            target = pickWanderTarget(goose, ctx.getGroundY, ctx.waterZones, ctx.solidObstacles, _rand);
          }
      ...
      case 'launched': {
        ...
        if (resolvePositionAgainstBuildings(pos, 0.3, ctx.buildingColliders)) {
          const speed = Math.sqrt(goose.velX * goose.velX + goose.velZ * goose.velZ);
          if (speed > 0.001) {
            goose.velX *= -6 / speed;
            goose.velZ *= -6 / speed;
          }
...
function checkScoop(goose: Goose, ctx: GooseUpdateContext, result: GooseUpdateResult): boolean {
  ...
  if (busDist < GOOSE_SCOOP_DISTANCE) {
    ...
    goose.velX = fwdX * ctx.busSpeed * SCOOP_FORWARD_FACTOR + (_rand() - 0.5) * 3;
    goose.velY = Math.max(SCOOP_MIN_UP, absSpeed * SCOOP_UP_FACTOR) + _rand() * 3;
    goose.velZ = fwdZ * ctx.busSpeed * SCOOP_FORWARD_FACTOR + (_rand() - 0.5) * 3;
    goose.herdId = -1;
