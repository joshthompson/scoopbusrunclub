...
export function buildBridgeMeshes(
  ...
  for (let i = 0; i < bridges.length; i++) {
    ...
    for (const side of [-1, 1]) {
      const kerb = MeshBuilder.CreateBox(`bridge_kerb_${i}_${side}`, {
        width: KERB_WIDTH,
        height: KERB_HEIGHT,
        depth: length,
      }, scene);
    ...
    for (const side of [-1, 1]) {
      ...
      const topRail = MeshBuilder.CreateBox(`bridge_rail_${i}_${side}`, {
      ...
      topRail.position.set(
        railX,
        DECK_THICKNESS / 2 + RAILING_HEIGHT,
        0,
      );

      // Mid rail
      const midRail = MeshBuilder.CreateBox(`bridge_midrail_${i}_${side}`, {
        width: RAILING_POST_THICKNESS,
        height: RAILING_POST_THICKNESS,
        depth: length,
      }, scene);
      ...
      midRail.position.set(
        railX,
        DECK_THICKNESS / 2 + RAILING_HEIGHT * 0.5,
        0,
      );
      ...
      for (let p = 0; p < numPosts; p++) {
        ...
        const post = MeshBuilder.CreateBox(`bridge_post_${i}_${side}_${p}`, {
          width: RAILING_POST_THICKNESS,
        ...
        post.position.set(
          railX,
          DECK_THICKNESS / 2 + RAILING_HEIGHT / 2,
          postZ,
        );
      }
    }
...
export function resolveBridgeCollision(
  ...
  const resolvedRight = (localRight >= 0 ? 1 : -1) * (collider.halfWidth + radius);
...
export function getBridgeDeckY(
  ...
  for (const collider of colliders) {
    ...
    if (Math.abs(localFwd) <= collider.halfLength && Math.abs(localRight) <= collider.halfWidth) {
