 ...
 */
export function createBoostEffects(params: CreateBoostEffectsParams): BoostEffectsInstance {
  const { scene, camera, particleLayerMask, enableDepthOfField = false } = params;
  ...
  return {
    update(dt: number, boostIntensity: number, elapsed: number) {
      ...
      if (target > smoothIntensity) {
        smoothIntensity += (target - smoothIntensity) * Math.min(1, FADE_IN_SPEED * dt);
      } else {
        smoothIntensity += (target - smoothIntensity) * Math.min(1, FADE_OUT_SPEED * dt);
      }
...
function applyShake(camera: FreeCamera, intensity: number, elapsed: number): void {
...
function createSpeedLineParticles(
  ...
  const tex = new DynamicTexture('speedLineTex', { width: 8, height: 64 }, scene, false);
  ...
  const emitter = MeshBuilder.CreateBox('speedLineEmitter', { size: 0.01 }, scene);
...
function createRadialBlurPostProcess(scene: Scene, camera: FreeCamera): RadialBlurHandle {
