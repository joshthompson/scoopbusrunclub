...
import { createDeerModel, poseDeerWalking, poseDeerIdle, poseDeerSitting, poseDeerFleeing, poseDeerRunning } from '../objects/DeerModel';
...
function updateHerds(deer: Deer[]) {
  ...
  const active = deer.filter(d => d.state !== 'launched' && d.state !== 'landed');
...
function getHerdTarget(deer: Deer[], herdId: number): { tx: number; tz: number } | null {
  for (const d of deer) {
    if (d.herdId === herdId && (d.state === 'walking' || d.state === 'running')) {
        ...
        if (deer.idleTimer <= 0) {
          ...
          if (deer.herdId !== -1) {
            ...
            if (herdTarget) {
              ...
              target = pickWanderTarget(deer, ctx.getGroundY, ctx.waterZones, ctx.solidObstacles, _rand);
            }
          } else {
            target = pickWanderTarget(deer, ctx.getGroundY, ctx.waterZones, ctx.solidObstacles, _rand);
          }
...
function checkScoop(deer: Deer, ctx: DeerUpdateContext, result: DeerUpdateResult): boolean {
