 ...
 */
export const BUS_INERTIA =
	(BUS_MASS / 12) * ((2 * BUS_HALF_WIDTH) ** 2 + (2 * BUS_HALF_LENGTH) ** 2)
...
export function resolveBusToBusCollisions(
	...
	for (const remote of remotes) {
		const remoteCorners = getCorners(
			remote.x,
			remote.z,
			remote.yaw,
			BUS_HALF_WIDTH,
		...
		const jRaw =
			(-(1 + BUS_RESTITUTION) * relVelNormal) /
			(invMassSum + angularTermLocal + angularTermRemote)
		...
		const clampedAngular = Math.max(
			-BUS_MAX_ANGULAR_IMPULSE,
			Math.min(BUS_MAX_ANGULAR_IMPULSE, angularImpulse),
		)
		local.yawRate += clampedAngular
		// Clamp total yaw rate
		local.yawRate = Math.max(
			-BUS_MAX_YAW_RATE,
			Math.min(BUS_MAX_YAW_RATE, local.yawRate),
		)
		...
		if (remote.id) {
			...
			const clampedRemoteAngular =
				Math.max(
					-BUS_MAX_ANGULAR_IMPULSE,
					Math.min(BUS_MAX_ANGULAR_IMPULSE, remoteAngularImpulse),
				) * nudgeScale
			remoteNudges.push({
				id: remote.id,
