...
function resolveMinimapColor(
	charSel: CharacterSelection | null | undefined,
	playerIndex: number,
): string {
...
export function buildMinimapPlayers(
	params: BuildMinimapPlayersParams,
): MinimapPlayer[] {
	const { remotePlayers, localPlayerIndex, busPos, busYaw, charSelection } =
		params
	...
	for (const [_peerId, remote] of remotePlayers) {
		...
		const color = remoteCharSel
			...
			: color3ToHex(
					(PLAYER_COLORS[remote.playerIndex - 1] ?? PLAYER_COLORS[1]).body,
				)
		players.push({
