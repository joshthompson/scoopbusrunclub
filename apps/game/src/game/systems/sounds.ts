...
function updateBusPairVolume(
	...
	if (
		...
		const fadeProgress =
			(currentTime - (duration - CROSSFADE_DURATION)) / CROSSFADE_DURATION
...
export function updateGameSounds(
	...
	for (let i = 0; i < buses.length; i++) {
		...
		if (absSpeed <= BUS_ENGINE_NORMAL_MAX_SPEED) {
			...
			const t = Math.min(
				(absSpeed - BUS_ENGINE_NORMAL_MAX_SPEED) /
					(BUS_ENGINE_BOOST_MAX_SPEED - BUS_ENGINE_NORMAL_MAX_SPEED),
				1,
			)
			speedScale =
				BUS_ENGINE_SPEED_MULTIPLIER +
				t * (BUS_ENGINE_BOOST_MULTIPLIER - BUS_ENGINE_SPEED_MULTIPLIER)
