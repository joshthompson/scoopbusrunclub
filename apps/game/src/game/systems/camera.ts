...
export function updateDemoCameraSystem(params: DemoCameraParams): number {
	...
	let demoCamProgress =
		params.demoCamProgress + (DEMO_CAMERA_SPEED * dt) / pathTotalDistance
...
export function updateCountdownCameraSystem(
	params: CountdownCameraParams,
): void {
	const {
		dt,
		camera,
...
export function updateChaseCameraSystem(params: ChaseCameraParams): number {
	const { dt, camera, busYaw, busSpeed, busPos, getGroundY, getWaterSurfaceY } =
		params
	...
	const targetFov =
		CAMERA_FOV_BUS + Math.abs(busSpeed) * CAMERA_FOV_SPEED_FACTOR
	camera.fov +=
		(targetFov - camera.fov) * Math.min(1, CAMERA_FOV_LERP_SPEED * dt)

	const targetYawOffset = busSpeed < -1 ? Math.PI : 0
	const camSwingSpeed = 3
	const yawDiff = targetYawOffset - params.cameraYawOffset
	const cameraYawOffset =
		params.cameraYawOffset +
...
export function updateRunnerCameraSystem(params: RunnerCameraParams): void {
	const { dt, camera, runnerYaw, runnerPos, getGroundY, getWaterSurfaceY } =
		params
