...
function samplePathAtDist(
	...
	if (positions.length < 2)
		return { x: positions[0]?.[0] ?? 0, z: positions[0]?.[1] ?? 0 }
...
export class PassengerSystem {
	...
	private isInsideFence(x: number, z: number): boolean {
		...
		return (
			dx * dx + dz * dz <= this.fenceBounds.radius * this.fenceBounds.radius
		)
	}
	...
	private randomPositionInFence() {
		for (let attempt = 0; attempt < 50; attempt++) {
			const pos = randomPositionOnAnyPath(
				this.allPaths,
				this.getGroundY,
				this.rand,
			)
		...
		for (const p of this.passengers) {
			...
			switch (p.state) {
				case 'waiting': {
					...
					if (
						dist < PASSENGER_PICKUP_SCOOP_DISTANCE &&
						Math.abs(busSpeed) > 0.5
					) {
				...
				case 'launched': {
					...
					const seatWorldX =
						busPos.x + cosY * p.ridingOffsetX + sinY * afterPitchZ
					const seatWorldZ =
						busPos.z - sinY * p.ridingOffsetX + cosY * afterPitchZ
	...
	private assignRoofSeat(newRider: Passenger) {
		...
		for (let i = 0; i < allRiders.length; i++) {
			...
			allRiders[i].ridingOffsetX =
				cols > 1 ? -BUS_ROOF_W / 2 + col * spacingX : 0
			allRiders[i].ridingOffsetZ =
				rows > 1 ? -BUS_ROOF_L / 2 + row * spacingZ : 0
		}
	}
	...
	private deliverPassenger(p: Passenger, busPos: Vector3) {
		...
		const bonus = Math.max(
			PASSENGER_PICKUP_DROPOFF_BONUS_MIN,
			PASSENGER_PICKUP_DROPOFF_BONUS_START -
				this.deliveries * PASSENGER_PICKUP_DROPOFF_BONUS_DECAY,
		)
		this.deliveries++
		this.timer += bonus
		this.callbacks.onTimerChange(this.timer)
	...
	private createFlag(
		...
		const banner = MeshBuilder.CreateGround(
			`flagBanner_${index}`,
			{
				width: FLAG_BANNER_WIDTH,
				height: FLAG_BANNER_HEIGHT,
				subdivisions: FLAG_BANNER_SUBDIVISIONS,
				updatable: true,
			},
			this.scene,
		)
		...
		this.scene.registerBeforeRender(() => {
			...
			for (let i = 0; i < positions.length; i += 3) {
				...
				positions[i + 1] =
					restPositions[i + 1] +
					Math.sin(t * 4 + nx * 6 + bannerIdx) * amplitude
			}
