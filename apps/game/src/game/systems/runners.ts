...
export function updateRunnersSystem(
	...
	for (const runner of ctx.runners) {
		...
		const animateRunner =
			distToLocalX * distToLocalX + distToLocalZ * distToLocalZ <=
			animationCullDistanceSq

		switch (runner.state) {
			case 'running': {
				const pathCount = ctx.pathPositions.length
				const target = ctx.pathPositions[runner.targetIdx]
				if (!target) break

				const prevIdx = Math.max(0, runner.targetIdx - 1)
				const nextIdx = Math.min(pathCount - 1, runner.targetIdx + 1)
				const segStart = runner.returningToStart
					? ctx.pathPositions[prevIdx]
					: target
				const segEnd = runner.returningToStart
					? target
					: ctx.pathPositions[nextIdx]
				const segDx = segEnd[0] - segStart[0]
				const segDz = segEnd[1] - segStart[1]
				...
				if (ctx.localPlayerRole === 'bus' && absSpeed > 0.5) {
					...
					if (alongDist > 0 && alongDist < absSpeed * RUNNER_ESCAPE_LOOKAHEAD) {
						...
						const crossDist = Math.abs(
							toRunnerX * busMoveDirZ - toRunnerZ * busMoveDirX,
						)
						if (
							crossDist <
							RUNNER_ESCAPE_NEAR_MISS_RADIUS + BUS_COLLISION_RADIUS
						) {
							busWillHit = true
						}
					}
				...
				if (busWillHit) {
					...
					const escWaterY = getWaterSurfaceYAt(pos.x, pos.z, ctx.waterZones)
					pos.y =
						escWaterY !== null
							? escWaterY - RUNNER_WATER_SINK
							: ctx.getGroundY(pos.x, pos.z)
					runner.mesh.rotation.y = Math.atan2(escapeX, escapeZ)

					if (animateRunner) {
						if (escWaterY !== null) {
							runner.animPhase += dt * RUNNER_ESCAPE_SPEED * 1
							poseSwimming(runner.model, runner.animPhase)
							runner.model.root.rotation.x +=
								(RUNNER_SWIM_TILT - runner.model.root.rotation.x) *
								Math.min(1, RUNNER_SWIM_TILT_SPEED * dt)
						} else {
					...
					if (dist < 1) {
						if (runner.returningToStart) {
							runner.targetIdx = Math.max(0, runner.targetIdx - 1)
							if (runner.targetIdx === 0) {
								runner.returningToStart = false
							}
						} else {
							runner.targetIdx += 1
							if (runner.targetIdx >= pathCount) {
								// Finished: run back to the start along the same path instead of teleporting.
								runner.targetIdx = pathCount - 1
								runner.returningToStart = true
							}
						}
					} else {
						...
						const runningSpeedMultiplier =
							runnerSlope < RUNNER_DOWNHILL_SLOPE_THRESHOLD
								? RUNNER_DOWNHILL_SPEED_BOOST
								: 1
					...
					const runWaterY = getWaterSurfaceYAt(pos.x, pos.z, ctx.waterZones)
					pos.y =
						runWaterY !== null
							? runWaterY - RUNNER_WATER_SINK
							: ctx.getGroundY(pos.x, pos.z)
					if (animateRunner) {
						if (runWaterY !== null) {
							runner.animPhase += dt * runner.speed * 1
							poseSwimming(runner.model, runner.animPhase)
							runner.model.root.rotation.x +=
								(RUNNER_SWIM_TILT - runner.model.root.rotation.x) *
								Math.min(1, RUNNER_SWIM_TILT_SPEED * dt)
			...
			case 'launched': {
				...
				if (MODE === 'SCOOP_THEN_RIDE') {
					...
					const seatWorldX =
						scooperBusPos.x + cosY * runner.ridingOffsetX + sinY * afterPitchZL
					const seatWorldZ =
						scooperBusPos.z - sinY * runner.ridingOffsetX + cosY * afterPitchZL
							...
							if (obs.elasticIndex != null) {
								...
								if (elastic) {
									const speed = Math.sqrt(
										runner.velX * runner.velX + runner.velZ * runner.velZ,
									)
					...
					if (pos.y <= roofWorldY && runner.velY < 0) {
						runner.state = 'riding'
						runner.ridingYawOffset =
							(Math.random() - 0.5) * ((40 * Math.PI) / 180) // ±20°
					...
					const landY =
						launchWaterY !== null
							? Math.max(groundY, launchWaterY - RUNNER_WATER_SINK)
							: groundY
					if (pos.y <= landY && runner.velY < 0) {
						pos.y = landY
			...
			case 'riding': {
				...
				pos.x =
					riderBusPos.x + cosY * runner.ridingOffsetX + sinY * afterPitchZR
				pos.z =
					riderBusPos.z - sinY * runner.ridingOffsetX + cosY * afterPitchZR
			...
			case 'sitting': {
				const sitWaterY = getWaterSurfaceYAt(pos.x, pos.z, ctx.waterZones)
				pos.y =
					sitWaterY !== null
						? sitWaterY - RUNNER_WATER_SINK - RUNNER_WATER_SINK_IDLE_EXTRA
						: ctx.getGroundY(pos.x, pos.z)
				...
				if (runner.fadeTimer <= 0) {
					...
					if (bestIdx >= ctx.pathPositions.length - 1) {
						runner.targetIdx = ctx.pathPositions.length - 1
						runner.returningToStart = true
					} else {
						runner.targetIdx = bestIdx + 1
					}
				}
				break
			}
 ...
 */
export function assignRoofSeat(
	allRunners: Runner[],
	localPlayerIndex: number,
	newRider: Runner,
) {
	const riders = allRunners.filter(
		(r) =>
			(r.state === 'riding' || (r.state === 'launched' && r !== newRider)) &&
			r.ownerPlayerIndex === localPlayerIndex,
	)
 ...
 */
export function packRemoteRiders(remote: {
	riderAnchors: Mesh[]
	smoothPos: Vector3
	smoothYaw: number
}) {
...
export function updateLocalRunnerVisual(
	...
	// Gradually lerp body tilt toward target (70° when swimming, 0° otherwise)
	const targetTilt =
		ctx.inWater && Math.abs(ctx.busSpeed) > 0.15 ? RUNNER_SWIM_TILT : 0
	const currentTilt = ctx.model.root.rotation.x
	const tiltDiff = targetTilt - currentTilt
	ctx.model.root.rotation.x =
		currentTilt + tiltDiff * Math.min(1, RUNNER_SWIM_TILT_SPEED * dt)
