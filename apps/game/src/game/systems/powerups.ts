...
export class PowerUpSystem {
	...
	private onPowerUpDisplayChange?: (
	...
	constructor(opts: {
		...
		const minSpacingSq =
			POWER_UP_MIN_TOKEN_SPACING_METRES * POWER_UP_MIN_TOKEN_SPACING_METRES
		const startX = opts.pathPositions[0][0]
		const startZ = opts.pathPositions[0][1]
		const startExclusionSq =
			POWER_UP_START_EXCLUSION_METRES * POWER_UP_START_EXCLUSION_METRES

		let tokenId = 0
		for (
			let dist = POWER_UP_ITEM_SPACING;
			dist < totalDist;
			dist += POWER_UP_ITEM_SPACING
		) {
			...
			const y =
				opts.getGroundY(sample.x, sample.z) + POWER_UP_TOKEN_FLOAT_HEIGHT
			const mesh = MeshBuilder.CreateDisc(
				`powerUpToken_${tokenId}`,
				{
					radius: POWER_UP_TOKEN_RADIUS,
					tessellation: 32,
	...
	update(nowMs: number) {
		...
		for (const token of this.tokens) {
			...
			token.mesh.position.y =
				token.baseY +
				Math.sin(t * POWER_UP_TOKEN_FLOAT_SPEED + token.phaseOffset) *
					POWER_UP_TOKEN_FLOAT_AMPLITUDE
		}
	}
	...
	private startPowerUpRoll(nowMs: number) {
		...
		this.selectedPowerUp =
			POWER_UP_IDS[Math.floor(Math.random() * POWER_UP_IDS.length)]
