...
export interface PathShaderOptions {
	...
	isNight?: boolean
	/** Per-segment path type info: which texture to use for each segment of the path */
	pathTypeSegments?: PathTypeSegment[]
	/** Per-point width multipliers (1 = default width). Indexed same as pathPositions. */
	pathWidths?: number[]
}

/** Describes a contiguous section of the path with a specific surface type */
export interface PathTypeSegment {
	/** Start index into pathPositions (inclusive) */
	startIndex: number
	/** End index into pathPositions (inclusive) */
	endIndex: number
