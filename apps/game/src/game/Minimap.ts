...
const COL_BUILDING_BRICK = '#b05838'
...
export class Minimap {
	...
	private gapThresholdSq = Number.POSITIVE_INFINITY // adaptive gap detection
	...
	private computeGapThreshold() {
		...
		if (pts.length < 3) {
			this.gapThresholdSq = Number.POSITIVE_INFINITY
			return
		}
	...
	private findClosestPathDistance(x: number, z: number): number {
		if (
			...
			this.pathCumDist.length !== this.pathPositions.length
		)
			return 0
	...
	private samplePathAtDistance(pathDist: number): [number, number] | null {
		if (
			this.pathPositions.length < 2 ||
			this.pathCumDist.length < 2 ||
			this.pathTotalDist <= 0
		)
			return null
		...
		const [bx, bz] =
			this.pathPositions[segIdx + 1] ?? this.pathPositions[segIdx]
	...
	private drawLookaheadSegment(
		...
		if (nextGatePathDist != null && playerPathDist != null) {
			...
			endDist = Math.min(
				this.pathTotalDist,
				nextGatePathDist + LOOKAHEAD_SEGMENT_METRES * 0.3,
			)
		} else {
			...
			endDist = Math.min(
				this.pathTotalDist,
				anchorDist + LOOKAHEAD_SEGMENT_METRES,
			)
		}
