...
export type TiledPathGroundMaterial = ShaderMaterial & {
	...
	__setShadowMap?: (
		...
		cam: { getViewMatrix(): Matrix; getProjectionMatrix(): Matrix },
		light: SpotLight,
	) => void
}
...
export function createTiledPathGroundMaterial(
	...
	const midMask = bakeMask(
		...
		startLine,
		startCircle,
		initialCX - halfMidInset,
		initialCZ - halfMidInset,
		MID_INSET_SIZE,
		MID_INSET_SIZE,
		regions,
		waterZones,
	...
	const ultraMask = bakeMask(
		...
		startLine,
		startCircle,
		initialCX - halfUltraInset,
		initialCZ - halfUltraInset,
		ULTRA_INSET_SIZE,
		ULTRA_INSET_SIZE,
		regions,
		waterZones,
	...
	const grassPathTex = new Texture(grassPathUrl, scene)
	grassPathTex.anisotropicFilteringLevel = RENDER_TEXTURE_ANISOTROPY

	const gravelTex = new Texture(gravelUrl, scene)
	gravelTex.anisotropicFilteringLevel = RENDER_TEXTURE_ANISOTROPY

	const pavementTex = new Texture(pavementUrl, scene)
	pavementTex.anisotropicFilteringLevel = RENDER_TEXTURE_ANISOTROPY
	...
	// Set mask textures (3 LOD levels × 4 mask types)
	...
	mat.setTexture('grassPathTex', grassPathTex)
	mat.setTexture('gravelTex', gravelTex)
	mat.setTexture('pavementTex', pavementTex)
	...
	mat.setVector3('sandColorVal', hexToVec3('#d4c49a'))
	...
	mat.setVector4(
		'midBounds',
		vec4(midBounds[0], midBounds[1], midBounds[2], midBounds[3]),
	)
	mat.setVector4(
		'ultraBounds',
		vec4(
	...
	const sunLen = Math.sqrt(
		sunDir.r * sunDir.r + sunDir.g * sunDir.g + sunDir.b * sunDir.b,
	)
	mat.setVector3('sunDirection', {
		x: sunDir.r / sunLen,
		y: sunDir.g / sunLen,
		z: sunDir.b / sunLen,
	} as any)
	mat.setFloat(
		'sunIntensity',
		isNight ? LIGHT_NIGHT_TERRAIN_SUN_INTENSITY : LIGHT_DAY_SUN_INTENSITY,
	)
	mat.setFloat(
		'hemiIntensity',
		isNight ? LIGHT_NIGHT_TERRAIN_HEMI_INTENSITY : LIGHT_DAY_HEMI_INTENSITY,
	)
	mat.setVector3(
		'hemiGroundColor',
		isNight
	...
	mat.onBind = () => {
		...
		for (let li = 0; li < lights.length && count < MAX_SPOTS; li++) {
			...
			if (light.parent) {
				Vector3.TransformNormalToRef(
					light.direction,
					(light.parent as TransformNode).getWorldMatrix(),
					_tmpDir,
				)
				_tmpDir.normalize()
		...
		if (effect) {
			...
			if (count > 0) {
				effect.setArray3(
					'spotPositions',
					Array.from(spotPosArr.subarray(0, count * 3)),
				)
				effect.setArray3(
					'spotDirections',
					Array.from(spotDirArr.subarray(0, count * 3)),
				)
				effect.setArray3(
					'spotColors',
					Array.from(spotColArr.subarray(0, count * 3)),
				)
			...
			if (shadowRTT && shadowCamera && shadowIdx >= 0) {
				shadowCamera
					.getViewMatrix()
					.multiplyToRef(shadowCamera.getProjectionMatrix(), _shadowVP)
	...
	const updateInsetCenter = (playerX: number, playerZ: number) => {
		...
		if (mdx * mdx + mdz * mdz >= MID_REBAKE_THRESHOLD * MID_REBAKE_THRESHOLD) {
			...
			rebakeMask(
				...
				startLine,
				startCircle,
				playerX - halfMidInset,
				playerZ - halfMidInset,
				MID_INSET_SIZE,
				MID_INSET_SIZE,
				regions,
				waterZones,
		...
		if (
			udx * udx + udz * udz >=
			ULTRA_REBAKE_THRESHOLD * ULTRA_REBAKE_THRESHOLD
		) {
			...
			rebakeMask(
				...
				startLine,
				startCircle,
				playerX - halfUltraInset,
				playerZ - halfUltraInset,
				ULTRA_INSET_SIZE,
				ULTRA_INSET_SIZE,
				regions,
				waterZones,
...
interface BakedMask {
	...
	pathTypeMaskTex?: DynamicTexture
...
function bakeMask(
	...
	const lineMaskTex = new DynamicTexture(
		`lineMask_${label}`,
		size,
		scene,
		false,
	...
	const lineCtx =
		lineMaskTex.getContext() as unknown as CanvasRenderingContext2D
	const zoneMaskTex = new DynamicTexture(
		`zoneMask_${label}`,
		size,
		scene,
		false,
	...
	const zoneCtx =
		zoneMaskTex.getContext() as unknown as CanvasRenderingContext2D
	...
	let pathTypeMaskTex: DynamicTexture | undefined
	if (bakePathType) {
		pathTypeMaskTex = new DynamicTexture(
			`pathTypeMask_${label}`,
			size,
			scene,
			false,
		...
		const pathTypeCtx =
			pathTypeMaskTex.getContext() as unknown as CanvasRenderingContext2D
		pathTypeMaskTex.anisotropicFilteringLevel = RENDER_TEXTURE_ANISOTROPY
		paintPathTypeMask(
			pathTypeCtx,
			size,
			pathPositions,
...
function rebakeMask(
	...
	const lineCtx =
		mask.lineMaskTex.getContext() as unknown as CanvasRenderingContext2D
	const zoneCtx =
		mask.zoneMaskTex.getContext() as unknown as CanvasRenderingContext2D
...
const CORNER_CURVE_OFFSET = 10
/** Max fraction of a line segment length consumed by curve on each side */
const MAX_CURVE_FRACTION = 0.35

/**
 * Draw a path with smoothly varying width using filled circles at each point
 * and trapezoid quads between consecutive points. This provides smooth
 * linear interpolation of width between points, avoiding step artifacts.
 */
function drawVariableWidthPath(
	ctx: CanvasRenderingContext2D,
	points: [number, number][],
	/** Per-point pixel half-widths */
	halfWidths: number[],
	color: string,
	...
	// Draw filled circles at each point for smooth joins
	...
	// Draw trapezoid quads between consecutive points
 ...
 * Instead of straight lineTo through each vertex, the path stops
 * CORNER_CURVE_OFFSET metres (max MAX_CURVE_FRACTION of segment length)
 * before each corner and curves through to the same offset after.
 * Only applies curves to interior vertices (not the course endpoints).
 */
function drawSmoothedPolyline(
	...
	for (let i = 1; i < points.length - 1; i++) {
		...
		const offsetBefore = Math.min(
			CORNER_CURVE_OFFSET,
			prevLen * MAX_CURVE_FRACTION,
		)
		const offsetAfter = Math.min(
			CORNER_CURVE_OFFSET,
			nextLen * MAX_CURVE_FRACTION,
		)

		// Point before corner: along segment from prev toward corner
		const [cx, cz] = points[i]
		const [prevX, prevZ] = points[i - 1]
		const [nextX, nextZ] = points[i + 1]

		// Direction from prev to corner (normalised)
		...
		// Direction from corner to next (normalised)
...
function paintMaskContent(
	...
	if (waterZones.length > 0) {
		...
		for (const wz of waterZones) {
			...
			const encodedY = Math.max(
				0,
				Math.min(255, Math.round(((wz.y + 100) / 200) * 255)),
			)
	...
	const drawPolylineStroke = (
		...
		targetCtx.beginPath()
		drawSmoothedPolyline(targetCtx, points, toPixel)
		targetCtx.stroke()
	}
	...
	const hasVariableWidths =
		pathWidths.length >= pathPositions.length && pathWidths.some((w) => w !== 1)

	ctx.globalCompositeOperation = 'lighten'
	if (hasVariableWidths && pathPositions.length >= 2) {
		// Smooth variable-width path using filled circles + trapezoid quads
		const outerHWs = pathPositions.map((_, i) =>
			worldToPixelW(pathHalfWidth * (pathWidths[i] ?? 1) + edgeSoftness),
		)
		const midHWs = pathPositions.map((_, i) =>
			worldToPixelW(pathHalfWidth * (pathWidths[i] ?? 1) + edgeSoftness * 0.4),
		)
		const coreHWs = pathPositions.map((_, i) =>
			worldToPixelW(pathHalfWidth * (pathWidths[i] ?? 1)),
		)
	...
	if (startCircle && pathPositions.length > 0) {
		...
		if (dist > 0.001) {
			...
			const outerStart = toPixel(
				startCircle.x +
					dirX *
						Math.max(0, startCircle.radius - (pathHalfWidth + edgeSoftness)),
				startCircle.z +
					dirZ *
						Math.max(0, startCircle.radius - (pathHalfWidth + edgeSoftness)),
			)
			const midStart = toPixel(
				startCircle.x +
					dirX *
						Math.max(
							0,
							startCircle.radius - (pathHalfWidth + edgeSoftness * 0.4),
						),
				startCircle.z +
					dirZ *
						Math.max(
...
// ---------- Path type mask painting ----------

/**
 * Paint the path type mask texture.
 * R = gravel, G = pavement, B = grass-path.
 * Where all channels are 0, the default dirt texture is used.
 */
function paintPathTypeMask(
	ctx: CanvasRenderingContext2D,
	size: number,
	pathPositions: [number, number][],
	pathHalfWidth: number,
	...
	pathTypeSegments: PathTypeSegment[],
	pathWidths: number[],
	startCircle?: { x: number; z: number; radius: number },
) {
	// Start black (all zero = dirt default)
	ctx.fillStyle = 'rgb(0, 0, 0)'
	ctx.fillRect(0, 0, size, size)

	if (pathTypeSegments.length === 0 || pathPositions.length < 2) return

	const toPixel = makeToPixelFn(worldMinX, worldMinZ, worldW, worldH, size)
	const worldToPixelW = (w: number) => (w / worldW) * size

	// Determine the path type at index 0 (for the start circle)
	let startType: PathTypeSegment['type'] | null = null
	for (const seg of pathTypeSegments) {
		if (seg.startIndex <= 0 && seg.endIndex >= 0) {
			startType = seg.type
			break
		}
	}

	// For each segment, paint the path section with the appropriate color channel
	for (const seg of pathTypeSegments) {
		const startIdx = Math.max(0, seg.startIndex)
		const endIdx = Math.min(pathPositions.length - 1, seg.endIndex)
		if (startIdx >= endIdx) continue

		// Determine the RGB color for this type
		let color: string
		switch (seg.type) {
			case 'gravel':
				color = 'rgb(255, 0, 0)'
		...
		// Use 'lighten' composite so overlapping segments merge
		ctx.globalCompositeOperation = 'lighten'

		// Extract per-point half-widths for this sub-path
		const subPath = pathPositions.slice(startIdx, endIdx + 1)

		if (pathWidths.length >= pathPositions.length) {
			// Smooth variable-width using filled circles + trapezoid quads
			const halfWidths = subPath.map((_, j) => {
				const w = pathWidths[startIdx + j] ?? 1
				return worldToPixelW(pathHalfWidth * w + edgeSoftness)
			})
			drawVariableWidthPath(ctx, subPath, halfWidths, color, toPixel)
		} else {
			// Uniform width
			const pixelWidth = worldToPixelW((pathHalfWidth + edgeSoftness) * 2)
			ctx.lineWidth = pixelWidth
			ctx.lineCap = 'round'
			ctx.lineJoin = 'round'
			ctx.strokeStyle = color
			...
			for (let i = 0; i < subPath.length; i++) {
				const [px, py] = toPixel(subPath[i][0], subPath[i][1])
	...
	// -- Start circle: paint with the first path's surface type --
	if (startCircle && startType) {
		let color: string
		switch (startType) {
			case 'gravel':
				color = 'rgb(255, 0, 0)'
		...
		const [cx, cy] = toPixel(startCircle.x, startCircle.z)
		const pixelRadius = worldToPixelW(startCircle.radius + edgeSoftness)
		...
		// Connector from circle to first path point
		if (pathPositions.length > 0) {
			const w0 = pathWidths.length > 0 ? (pathWidths[0] ?? 1) : 1
			const connWidth = worldToPixelW((pathHalfWidth * w0 + edgeSoftness) * 2)
			const [px, py] = toPixel(pathPositions[0][0], pathPositions[0][1])
