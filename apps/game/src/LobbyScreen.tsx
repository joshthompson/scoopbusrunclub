			...
			<div class="lobby-card">
				...
				<Show when={props.mode === 'host' || connected()}>
					<div class="lobby-game-type">
						...
						<span class="game-type-badge-desc">
							{GAME_TYPE_DESCRIPTIONS[gt()]}
						</span>
					</div>
				</Show>
				...
				<Show when={connected() && !canStart() && props.mode === 'host'}>
					<p class="lobby-warning">
						Need at least {modeConfig().minPlayers} players to start{' '}
						{GAME_TYPE_LABELS[gt()]}
					</p>
				</Show>
				...
				<Show when={connected()}>
					...
					<Show when={gt() === 'team-race'}>
						<div class="team-columns">
							<For each={['yellow', 'blue'] as TeamColor[]}>
								{(teamId) => {
									...
									const teamPlayers = () => {
										...
										return allEntries
											.filter(
												(e) => teamAssignments().get(e.idx)?.team === teamId,
											)
												...
												<For each={teamPlayers()}>
													{(entry) => {
														...
														return (
															...
															>
																<span>
																	{roleEmoji(role)} P{idx}{' '}
																	{
																		PLAYER_NAMES[
																			(idx - 1) % PLAYER_NAMES.length
																		]
																	}
																	{entry.isLocal ? ' (You)' : ''}
																</span>
						...
						<div class="lobby-players">
							...
							>
								{(entry) => {
									...
									const role = playerRole(idx)
									const color =
										PLAYER_CSS_COLORS[(idx - 1) % PLAYER_CSS_COLORS.length]
									const name = PLAYER_NAMES[(idx - 1) % PLAYER_NAMES.length]
									const isHost = props.mode === 'host'
									const showDriverBtn =
										isHost && modeConfig().hostCanAssignDriver
				...
				<div class="lobby-buttons">
					<Show when={connected() && props.mode === 'host'}>
						...
						>
							{canStart()
								? 'Start Game!'
								: `Need ${modeConfig().minPlayers}+ players`}
						</button>
					</Show>
