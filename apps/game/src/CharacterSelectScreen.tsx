...
function BusPreview(props: {
	...
	onMount(async () => {
		const url = await getBusPreviewUrl(
			props.colorId,
			props.bodyHex,
			props.scoopHex,
		)
	...
	return (
		<div class="runner-preview">
			<Show
				when={src()}
				fallback={
					<div class="bus-swatch" style={{ background: props.cssColor }} />
				}
			>
				<img
					src={src()}
...
function RunnerPreview(props: {
	preset: RunnerPreset
	hovered: boolean
	onLiveRef: (el: HTMLDivElement) => void
}) {
	const [src, setSrc] = createSignal<string>('')
	...
	return (
		<div class="runner-preview" ref={(el) => props.onLiveRef(el)}>
...
function RunnerTile(props: {
	...
	function handleMouseEnter() {
		...
		const appearance = isCorgiPreset(props.preset)
			? null
			: props.preset.appearance
...
export function CharacterSelectScreen(props: CharacterSelectScreenProps) {
	...
	const sortedRunnerPresets = () =>
		[...RUNNER_PRESETS].sort((a, b) => a.name.localeCompare(b.name))
	...
	const gridCount = () =>
		isBus() ? BUS_COLOR_OPTIONS.length + 1 : sortedRunnerPresets().length + 1
					...
					<div class="char-grid bus-grid" ref={gridRef}>
						...
						<button
							...
							classList={{
								...
								'menu-focused': isFocused(1 + BUS_COLOR_OPTIONS.length),
							}}
