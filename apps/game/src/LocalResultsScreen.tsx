...
export function LocalResultsScreen(props: LocalResultsScreenProps) {
		...
		const entries = [
			{
				...PLAYERS[0],
				role: props.p1Role,
				time: props.p1FinishTime,
				scored: props.p1Scored,
				idx: 1,
			},
			{
				...PLAYERS[1],
				role: props.p2Role,
				time: props.p2FinishTime,
				scored: props.p2Scored,
				idx: 2,
			},
		]
	...
	const bothFinished = () =>
		props.p1FinishTime !== null && props.p2FinishTime !== null
				...
				{winnerName() && (
					<p
						style={{
							...
							color: '#ffc107',
							'text-shadow':
								'0 2px 8px rgba(0,0,0,0.7), 0 0 12px rgba(255,200,0,0.4)',
							margin: '12px 0 20px',
						}}
					>
						{winnerName()}
