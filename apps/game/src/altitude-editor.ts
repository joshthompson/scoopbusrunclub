...
async function initEditor(levelId: string) {
	...
	altitudeData = [
		...level.altitude.map((p) => [...p] as [number, number, number]),
	]

	const course = level.course
	originCoord = [course.coordinates[0][0], course.coordinates[0][1]]

	const altitudePoints = level.altitude
	const elevations = course.coordinates.map((coord) => {
		const lon = coord[0]
		const lat = coord[1]
		let bestAlt = 0
	...
	localAltPoints = altitudeToLocal(
		altitudeData,
		originCoord,
		scaleFactor,
		elevationScale,
	...
	fieldPolygons = regionEntries
		.filter((r) => r.type === 'field')
		.map((r) => gpsToWorldPoly(r.points))
	concretePolygons = regionEntries
...
function buildGroundMesh(
	...
	try {
		...
		console.warn(
			'[altitude-editor] Terrain shader failed, falling back to solid color:',
			err,
		)
...
function applyTerrainHeights(mesh: Mesh) {
	...
	for (let i = 0; i < positions.length; i += 3) {
		...
		const waterInfo = getWaterDepressionAt(x, z, waterZones, (wx, wz) =>
...
function buildBuildingsSimple() {
	...
	const footprints = computeBuildingFootprintData(
		buildings,
		originCoord,
		scaleFactor,
	)
...
function buildObjectsSimple() {
	...
	const toPlaced = (
		items: [number, number, number][] | undefined,
	): PlacedObjectData[] => {
		if (!items) return []
...
function ensureTreeTemplates() {
	...
	_trunkTpl = MeshBuilder.CreateCylinder(
		'tpl_trunk',
		{
			height: 3.5,
			diameterTop: 0.3,
	...
	_crownTpl = MeshBuilder.CreateCylinder(
		'tpl_crown',
		{
			height: 5.5,
			diameterTop: 0,
	...
	if (groundMesh) {
		applyTerrainHeightsAsync(groundMesh).then(() => {
			const wireframeVisible = (
				document.getElementById('tog-wireframe') as HTMLInputElement
			).checked
...
function setupInput(canvas: HTMLCanvasElement) {
	...
	canvas.addEventListener('pointerup', (e) => {
		...
		if (wasClick) {
			...
			const now = performance.now()
			const dblClickDist = Math.hypot(
				e.clientX - lastClickPos.x,
				e.clientY - lastClickPos.y,
			)
...
function handleDoubleClick(e: PointerEvent) {
	...
	if (pick?.hit && pick.pickedPoint) {
		...
		const minElev =
			altitudeData.length > 0 ? Math.min(...altitudeData.map((p) => p[2])) : 0
...
function doRemesh() {
	...
	if (_remeshTimer !== null) {
		clearTimeout(_remeshTimer)
		_remeshTimer = null
	}
...
function setupToggles() {
	const bind = (
		id: string,
		getNode: () => TransformNode | Mesh | LinesMesh | null,
		extra?: (checked: boolean) => void,
	) => {
