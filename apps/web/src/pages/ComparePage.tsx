...
export function ComparePage(props: ComparePageProps) {
	...
	const runnerIds = createMemo(() => runnerDataList().map((d) => d?.id ?? ''))
	...
	const combinedRuns = createMemo(() => {
		...
		return ids.reduce((sum, id, i) => {
			const fromRunner = props.runners.find(
				(r) => r.parkrunId === id,
			)?.totalRuns
	...
	const finishesTogether = createMemo(() => {
		return sharedEvents().filter((ev) => {
			const seconds = ev.entries
				.filter((e) => e !== 'Volunteer')
	...
	const raceTableData = createMemo(() =>
		sharedEvents().map((ev) => {
			const formatted = new Date(`${ev.date}T00:00:00`).toLocaleDateString(
				'en-GB',
			...
			<div class={styles.container}>
				<FieldBlock>
					...
					<div class={styles.statsGrid}>
						<RunnerSummaryStat label="Combined runs">
							{combinedRuns()}
						</RunnerSummaryStat>
						<RunnerSummaryStat label="Combined events">
							{combinedEvents()}
						</RunnerSummaryStat>
						<RunnerSummaryStat label="Events together">
							{sharedEvents().length}
						</RunnerSummaryStat>
						<RunnerSummaryStat label="Volunteered together">
							{volunteeredTogether()}
						</RunnerSummaryStat>
						<RunnerSummaryStat label="Finishes within 10s">
							{finishesTogether()}
						</RunnerSummaryStat>
					</div>
				</FieldBlock>
							...
							<div class={styles.addGrid}>
								<For each={availableRunners()}>
									{(runner) => (
										...
										>
											<img
												src={runner.data.frames.face[0]}
												class={styles.addRunnerFace}
												alt={runner.data.name}
											/>
											<span class={styles.addRunnerName}>
												{runner.data.name}
											</span>
										</button>
									)}
										...
										<button
											class={
												canRemove()
													...
													: styles.removeRunnerBtnDisabled
											}
