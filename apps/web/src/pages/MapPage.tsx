...
function buildMapData(results: RunResultItem[], volunteers: VolunteerItem[]) {
	...
	const eventStats = new Map<
		string,
		{ name: string; country: string; runners: Set<string>; visits: Set<string> }
	>()
	...
	for (const r of results) {
		...
		if (!eventStats.has(r.event)) {
			eventStats.set(r.event, {
				name: displayEventName(r.event, r.eventName),
				country,
				runners: new Set(),
				visits: new Set(),
			})
	...
	for (const v of volunteers) {
		...
		if (!eventStats.has(v.event)) {
			eventStats.set(v.event, {
				name: displayEventName(v.event, v.eventName),
				country,
				runners: new Set(),
				visits: new Set(),
			})
...
const { lookup: COUNTRY_LOOKUP, doublePixels: DOUBLE_PIXELS } =
	getCountryLookup()
...
export function MapPage(props: MapPageProps) {
	const mapData = createMemo(() =>
		buildMapData(props.results, props.volunteers),
	)
	const visitedCountries = createMemo(
		() => new Set(mapData().map((cv) => cv.country)),
	)
	const totalCountries = createMemo(() => mapData().length)
	const totalEvents = createMemo(() =>
		mapData().reduce((sum, c) => sum + c.events.length, 0),
	)
	...
	const pixelRects = createMemo(() => {
		...
		for (const key of LAND_PIXELS) {
			...
			const fill = DOUBLE_PIXELS.has(key)
				? MAP_COLOR_DOUBLE_COUNTRY
				: colorForCountry(countryCode)
		...
		for (const [code, cells] of Object.entries(COUNTRY_PIXELS)) {
			...
			for (const [c, r] of cells) {
				...
				if (!LAND_PIXELS.has(key)) {
					const pixelFill = DOUBLE_PIXELS.has(key)
						? MAP_COLOR_DOUBLE_COUNTRY
						: fill
					rects.push({
						x: c * CELL,
							...
							return (
								...
								>
									...
									<Show
										when={data()}
										fallback={
											<div class={styles.tooltipLine}>Not yet visited</div>
										}
									>
										{(d) => (
											<>
												<div class={styles.tooltipLine}>
													{d().totalVisits} total visit
													{d().totalVisits !== 1 ? 's' : ''}
												</div>
												<div class={styles.tooltipLine}>
													{d().events.length} event
													{d().events.length !== 1 ? 's' : ''}
												</div>
											</>
										)}
				...
				<For each={mapData()}>
					{(cv) => (
						<div data-id={cv.country}>
							<DirtBlock
								title={`${COUNTRY_FLAGS[cv.country] ?? '🏳️'} ${COUNTRY_NAMES[cv.country] ?? cv.country}`}
							>
										...
										<For each={cv.events}>
											{(ev) => (
												<div class={styles.eventRow}>
													<A href={`/event/${ev.eventId}`} class={styles.link}>
														<strong>{ev.name}</strong>
													</A>
													<span class={styles.eventCount}>
														{ev.count} visit{ev.count !== 1 ? 's' : ''}
													</span>
