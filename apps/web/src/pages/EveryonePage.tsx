...
function computeGroupStats(
	...
	const totalDistanceKm =
		totalParkruns * PARKRUN_DISTANCE_KM +
		totalJuniorParkruns * JUNIOR_PARKRUN_DISTANCE_KM

	// -- Total runs (from runner records, which include pre-tracked runs) --
	const totalRunsFromRecords = runners.reduce(
		(sum, r) => sum + r.totalRuns + (r.totalJuniorRuns ?? 0),
		0,
	)
	...
	const avgAttendance =
		activeDates.size > 0
			? (results.length + volunteers.length) / activeDates.size
			: 0

	// -- Per-member distance contributions --
	const memberDistanceKm = new Map<string, number>()
	for (const r of results) {
		const km = isJuniorEvent(r.event)
			? JUNIOR_PARKRUN_DISTANCE_KM
			: PARKRUN_DISTANCE_KM
		memberDistanceKm.set(
			r.parkrunId,
			(memberDistanceKm.get(r.parkrunId) ?? 0) + km,
		)
	}

	// -- Close finishes (within 10s) --
	const dateEventKey = (r: RunResultItem) =>
		`${r.date}:${r.event}:${r.eventNumber}`
	...
	for (const eventResults of resultsByEvent.values()) {
		...
		for (let i = 0; i < sorted.length; i++) {
			for (let j = i + 1; j < sorted.length; j++) {
				const diff = Math.abs(
					parseTimeToSeconds(sorted[i].time) -
						parseTimeToSeconds(sorted[j].time),
				)
	...
	// -- Journey progress --
	const currentWaypointIdx = JOURNEY_WAYPOINTS.findIndex(
		(w) => w.km > totalDistanceKm,
	)
	const prevWaypoint =
		currentWaypointIdx > 0
			? JOURNEY_WAYPOINTS[currentWaypointIdx - 1]
			: JOURNEY_WAYPOINTS[0]
	const nextWaypoint =
		currentWaypointIdx >= 0
			? JOURNEY_WAYPOINTS[currentWaypointIdx]
			: JOURNEY_WAYPOINTS[JOURNEY_WAYPOINTS.length - 1]
	const segmentProgress =
		nextWaypoint.km > prevWaypoint.km
			? (totalDistanceKm - prevWaypoint.km) /
				(nextWaypoint.km - prevWaypoint.km)
			: 1
	const passedWaypoints = JOURNEY_WAYPOINTS.filter(
		(w) => w.km <= totalDistanceKm,
	)
...
function computeGroupCelebrations(
	results: RunResultItem[],
	runners: Runner[],
	volunteers: VolunteerItem[],
): GroupCelebration[] {
	...
	const distanceMilestones = [
		100, 500, 1000, 2500, 5000, 10000, 15000, 20000, 25000, 30000, 40000, 50000,
	]
...
export function EveryonePage(props: EveryonePageProps) {
	const stats = createMemo(() =>
		computeGroupStats(props.results, props.runners, props.volunteers),
	)
	const celebrations = createMemo(() =>
		computeGroupCelebrations(props.results, props.runners, props.volunteers),
	)
	...
	const totalMemberKm = createMemo(() =>
		memberContributions().reduce((s, m) => s + m.km, 0),
	)
	...
	return (
		<div class={styles.page}>
			...
			<FieldBlock title="Our Journey" signType="purple">
				<div class={styles.journeyHero}>
					...
					<div class={styles.subtitle}>
						If you lined up every parkrun we've ever done, our runs would
						stretch from Stockholm to…
					</div>
					...
					<div class={styles.journeyBar}>
						<div class={styles.journeyBarOuter}>
							<div class={styles.journeyBarTrack}>
								<div
									...
									style={{
										width: `${Math.min((stats().totalDistanceKm / stats().nextWaypoint.km) * 100, 100)}%`,
									}}
								/>
								...
								<For each={stats().passedWaypoints.filter((w) => w.km > 0)}>
									{(wp) => (
										<div
											...
											style={{
												left: `${(wp.km / stats().nextWaypoint.km) * 100}%`,
											}}
						...
						<div class={styles.journeyLabels}>
							<span>0 km</span>
							<span>
								{stats().nextWaypoint.emoji} {stats().nextWaypoint.name} (
								{stats().nextWaypoint.km.toLocaleString()} km)
							</span>
						</div>
						{/* Passed cities list */}
						...
						<div class={styles.journeyExplainer}>
							{(
								stats().nextWaypoint.km - stats().totalDistanceKm
							).toLocaleString()}{' '}
							km to go until {stats().nextWaypoint.name}!
						</div>
					</div>
				</div>
			</FieldBlock>
			...
			<div class={styles.statsGrid}>
				<RunnerSummaryStat label="Tracked parkruns">
					{stats().totalParkruns.toLocaleString()}
				</RunnerSummaryStat>
				<RunnerSummaryStat label="Different events">
					{stats().uniqueEvents}
				</RunnerSummaryStat>
				<RunnerSummaryStat label="Countries visited">
					{stats().uniqueCountries}
				</RunnerSummaryStat>
				<RunnerSummaryStat label="Active Saturdays">
					{stats().activeSaturdays}
				</RunnerSummaryStat>
				<RunnerSummaryStat label="Volunteer sessions">
					{stats().volunteerSessions}
				</RunnerSummaryStat>
				<RunnerSummaryStat label="Close finishes (≤10s)">
					{stats().closeFinishes}
				</RunnerSummaryStat>
				...
				<Show when={stats().busiestDate}>
					<RunnerSummaryStat label="Busiest Saturday" type="text">
						{formatDateDisplay(stats().busiestDate)} ({stats().busiestCount}{' '}
						members)
					</RunnerSummaryStat>
				</Show>
			</div>
				...
				<div class={styles.countryGrid}>
					<For each={PARKRUN_COUNTRIES}>
						{(country) => {
							...
							const events = () =>
								stats().countryEvents.get(country.code)?.size ?? 0
							return (
								<div
									class={styles.countryCard}
									classList={{ [styles.countryUnvisited]: !visited() }}
								>
				...
				<div class={styles.contributionsGrid}>
					<For each={memberContributions()}>
						{(member) => {
							...
							const pct =
								totalMemberKm() > 0 ? (member.km / totalMemberKm()) * 100 : 0
							return (
