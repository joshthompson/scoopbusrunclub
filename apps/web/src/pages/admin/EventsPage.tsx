...
export const EventsPage: Component = () => {
	...
	const [filterEventDate, setFilterEventDate] =
		createSignal<EventDateFilter>('upcoming')
	...
	const filteredRaces = createMemo(() => {
		...
		if (attendeeVal)
			list = list.filter((r) =>
				r.attendees.some((a) => a.runnerId === attendeeVal),
			)
					...
					<Show when={filtersOpen()}>
						<div class={styles.filters}>
							...
							<AdminSelect
								...
								onChange={(e) =>
									setFilterEventDate(e.currentTarget.value as EventDateFilter)
								}
							>
					...
					<AdminTable
						...
						data={(sortedRaces() ?? []).map((race) => [
							...
							<AdminDropdown>
								...
								<AdminDropdownItem onClick={() => handleDuplicate(race)}>
									Duplicate
								</AdminDropdownItem>
								<AdminDropdownItem onClick={() => handleDelete(race)}>
									Delete
								</AdminDropdownItem>
							</AdminDropdown>,
						])}
