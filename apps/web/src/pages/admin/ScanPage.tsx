							...
							<For each={todayRaces() ?? []}>
								{(race) => (
									<option value={race._id}>
										{race.name} - {race.date} ({race.attendees.length}{' '}
										attendees)
									</option>
								)}
							</For>
					...
					<Modal title="Runner Found!">
						<div class={styles.confirmContent}>
							<div class={styles.confirmText}>
								...
								<strong>{result().runnerName}</strong> to:
								<br />
								<strong>{result().eventName}</strong>
							</div>
							<div class={styles.confirmActions}>
								<AdminButton onClick={dismissCheckin} variant="secondary">
									Skip
								</AdminButton>
