...
export const UsersPage: Component = () => {
	...
	const handleEdit = async (e: Event) => {
		...
		try {
			...
			if (editIsSuperAdmin() !== user.isSuperAdmin)
				data.isSuperAdmin = editIsSuperAdmin()
	...
	return (
		...
		>
			<div>
				<AdminToolbar>
					<AdminButton onClick={() => setCreateModalOpen(true)}>
						+ Add User
					</AdminButton>
				</AdminToolbar>
					...
					<AdminTable
						...
						data={(users() ?? []).map((user) => [
							...
							<AdminDropdown>
								...
								<AdminDropdownItem
									onClick={() => alert('Do that in the database')}
								>
									Delete
								</AdminDropdownItem>
							</AdminDropdown>,
						...
						<form onSubmit={handleCreate} class={styles.form}>
							...
							<div class={styles.actions}>
								...
								<AdminButton
									type="submit"
									disabled={
										createSaving() || !createUsername() || !createPassword()
									}
								>
