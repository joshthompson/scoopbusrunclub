...
function isPbCelebration(name: string) {
	return (
		name === 'New PB!' || (name.startsWith('New ') && name.endsWith(' PB!'))
	)
}

function shouldShowCelebrationTime(name: string) {
	return (
		isPbCelebration(name) ||
		name === 'Palindrome!' ||
		name.endsWith('Palindrome Pal')
	)
}
		...
		<li class={styles.listItem}>
			<div class={styles.celebrationLine}>
				...
				<Show when={props.celebration.occurrences.length > 1}>
					{' '}
					× {props.celebration.occurrences.length}
				</Show>
			</div>
			<div class={styles.celebrationDescription}>
				{props.celebration.description}
			</div>
			...
			<Show when={latest()}>
				<div>
					...
					Achieved at{' '}
					<A href={`/event/${latest().event}`} class={styles.link}>
						{latest().eventName}
					</A>{' '}
					#{latest().eventNumber} on{' '}
					{formatDate(new Date(`${latest().date}T00:00:00`))}
					...
					<For each={earlier()}>
						{(occurrence) => (
							<div>
								...
								Achieved at{' '}
								<A href={`/event/${occurrence.event}`} class={styles.link}>
									{occurrence.eventName}
								</A>{' '}
								#{occurrence.eventNumber} on{' '}
...
export function MemberPage(props: MemberPageProps) {
	...
	const totalRuns = createMemo(
		() =>
			props.runners.find((runner) => runner.parkrunId === runnerId())
				?.totalRuns ?? runnerResults().length,
	)
	const totalJuniorRuns = createMemo(
		() =>
			props.runners.find((runner) => runner.parkrunId === runnerId())
				?.totalJuniorRuns ?? 0,
	)
	const totalRunsAtHaga = createMemo(
		() =>
			runnerResults().filter(
				(result) => result.event === 'haga' && result.parkrunId === runnerId(),
			).length,
	)
	const totalEvents = createMemo(
		() => new Set(runnerResults().map((result) => result.event)).size,
	)
	const runnerNameById = createMemo(() => {
		...
		for (const runner of props.runners) {
			if (!map.has(runner.parkrunId))
				map.set(runner.parkrunId, formatName(runner.name))
		}
		...
		createMemo(() => {
			...
			for (const myResult of runnerResults()) {
				...
				for (const otherResult of eventResults) {
					...
					if (withinSeconds !== Number.POSITIVE_INFINITY) {
					...
					nearbyCounts.set(
						otherResult.parkrunId,
						(nearbyCounts.get(otherResult.parkrunId) ?? 0) + 1,
					)
				}
			}
	...
	const groupedCelebrations = createMemo<GroupedCelebration[]>(() => {
		...
		const runnerVols = props.volunteers.filter(
			(v) => v.parkrunId === runnerId(),
		)
		for (const vol of runnerVols) {
			const tags = getVolunteerCelebrationTags(
				celebrationData(),
				vol.parkrunId,
				vol.date,
				vol.event,
				...
				<div class={styles.container}>
					<FieldBlock title={name()} signType="purple">
						...
						<div class={styles.runnerSummary}>
							...
							<div class={styles.runnerSummaryStats}>
								<RunnerSummaryStat label="Total runs">
									{totalRuns()}
								</RunnerSummaryStat>
								{totalJuniorRuns() > 0 && (
									<RunnerSummaryStat label="Junior runs">
										{totalJuniorRuns()}
									</RunnerSummaryStat>
								)}
								<RunnerSummaryStat label="Haga runs">
									{totalRunsAtHaga()}
								</RunnerSummaryStat>
								<RunnerSummaryStat label="Events">
									{totalEvents()}
								</RunnerSummaryStat>
								<div class={styles.pairedStats}>
									<RunnerSummaryStat label="Runs with" type="text">
										{oftenRunsWith(Number.POSITIVE_INFINITY)()}
									</RunnerSummaryStat>
									<RunnerSummaryStat label="Finishes with" type="text">
										{oftenRunsWith(30)()}
									</RunnerSummaryStat>
								</div>
							</div>
							{runner().id && (
								<div class={styles.parkrunBlock}>
									<a
										href={`https://www.parkrun.se/parkrunner/${runner().id}/all`}
										target="_blank"
					...
					<div class={styles.twoColumnGrid}>
						<DirtBlock title="Celebrations">
							<Show
								when={nonPbCelebrations().length > 0}
								fallback={<p>No celebrations yet.</p>}
							>
								<ul class={styles.list}>
									<For each={nonPbCelebrations()}>
										{(celebration) => (
											<AchievementItem celebration={celebration} />
										)}
									</For>
								</ul>
							</Show>
						...
						<DirtBlock title="PBs">
							<Show
								when={pbCelebrations().length > 0}
								fallback={<p>No PBs yet.</p>}
							>
								<ul class={styles.list}>
