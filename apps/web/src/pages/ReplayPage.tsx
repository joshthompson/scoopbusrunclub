...
for (const [key, [accessor]] of Object.entries(runnerSignals)) {
	...
	if (data.frames.face[0]) {
		...
		nameToFace.set(data.name.toLowerCase(), {
			face: data.frames.face[0],
			name: data.name,
		})
		if (data.altNames) {
			for (const alt of data.altNames) {
				nameToFace.set(alt.toLowerCase(), {
					face: data.frames.face[0],
					name: data.name,
				})
			}
...
export function ReplayPage(props: ReplayPageProps) {
	...
	const hasData = createMemo(
		() => replayRunners().length > 0 && courseData() !== null,
	)
	...
	const previewIframeSrc = createMemo(() => {
		...
		const runnerParams = runners
			.map((r) => {
				...
				return role
					? `${key}:${Math.round(r.finishSeconds)}:${role}`
					: `${key}:${Math.round(r.finishSeconds)}`
			})
			.filter(Boolean)
			.join(',')
	...
	const endPoint = createMemo<Point2D | null>(() => {
		...
		const p = projected()
		if (!cd || !p) return null
		const ep = cd.points.find((pt) =>
			END_POINTS.includes(pt.name.toLowerCase()),
		)
	...
	const runnerPositions = createMemo(() => {
		...
		return replayRunners().map((runner, idx) => {
			...
			if (finished && ep) {
				...
				const finishedBefore = replayRunners()
					.slice(0, idx)
					.filter((r) => t >= r.finishSeconds).length
	...
	// ---- Title ----
	const title = createMemo(() => `${eventName()} #${eventNumber()} Replay`)
	...
	return (
		<div class={styles.page}>
			<Show
				when={!courseLoading()}
				fallback={<p class={styles.loading}>Loading course…</p>}
			>
				<Show when={hasData()} fallback={<NotFoundPage />}>
					<FieldBlock title={title()}>
						<Show when={eventDate()}>
							{(date) => (
								<p class={styles.eventDate}>
							...
							<Show when={!show3D() || !has3DLevel()}>
											...
											>
												...
												>
													<image
														...
														transform={
															runner.facingRight
																? `translate(${runner.x * 2}, 0) scale(-1, 1)`
																: undefined
														}
													/>
												</Show>
								...
								<Show when={!COURSE_OVERRIDES[eventId()]}>
									<div class={styles.disclaimer}>
										The replay is based on finish times and map data which often
										doesn't reflect things like laps, direction or correct
										start/end locations.
									</div>
								</Show>
							</Show>
