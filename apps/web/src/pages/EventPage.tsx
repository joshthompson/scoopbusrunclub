...
export function EventPage(props: EventPageProps) {
	...
	const hasData = createMemo(
		() => eventResults().length > 0 || eventVolunteers().length > 0,
	)
	...
	const daysSinceFirst = createMemo(() => {
		...
		if (!f) return 0
		return Math.floor(
			(Date.now() - new Date(`${f}T00:00:00`).getTime()) / 86_400_000,
		)
	})
	...
	const runCountByRunner = createMemo(() => {
		const counts = new Map<
			string,
			{ count: number; name: string; parkrunId: string }
		>()
		for (const r of eventResults()) {
			const entry = counts.get(r.parkrunId) ?? {
				count: 0,
				name: r.runnerName,
	...
	const volunteerCounts = createMemo(() => {
		const counts = new Map<
			string,
			{ count: number; name: string; parkrunId: string; roles: Set<string> }
		>()
		for (const v of eventVolunteers()) {
			const entry = counts.get(v.parkrunId) ?? {
				count: 0,
	...
	const biggestCrowd = createMemo(() => {
		const counts = new Map<
			number,
			{ eventNumber: number; date: string; ids: Set<string> }
		>()
		for (const r of eventResults()) {
			if (!counts.has(r.eventNumber))
				counts.set(r.eventNumber, {
		...
		for (const entry of counts.values()) {
			if (!best || entry.ids.size > best.count)
				best = {
	...
	const theOGs = createMemo(() => {
		...
		if (allEntries.length === 0)
			return [] as { parkrunId: string; name: string; eventNumber: number }[]
		const minEvent = Math.min(...allEntries.map((e) => e.eventNumber))
		const seen = new Set<string>()
		return allEntries
			.filter((e) => e.eventNumber === minEvent)
			.filter((e) => {
				if (seen.has(e.parkrunId)) return false
				seen.add(e.parkrunId)
				return true
			})
	})
	...
	const bestBuddies = createMemo(() => {
		...
		const pairCounts = new Map<
			string,
			{ idA: string; idB: string; count: number }
		>()
	...
	const allParticipants = createMemo(() => {
		const map = new Map<
			string,
			{ parkrunId: string; name: string; runs: number; vols: number }
		>()
		for (const r of eventResults()) {
			const entry = map.get(r.parkrunId) ?? {
				parkrunId: r.parkrunId,
				name: r.runnerName,
				runs: 0,
				vols: 0,
		...
		for (const v of eventVolunteers()) {
			const entry = map.get(v.parkrunId) ?? {
				parkrunId: v.parkrunId,
				name: v.volunteerName,
				runs: 0,
				vols: 0,
	...
	const timeline = createMemo(() => {
		const groups = new Map<
			number,
			{
				eventNumber: number
				date: string
				runners: RunResultItem[]
				vols: VolunteerItem[]
			}
		>()
		for (const r of eventResults()) {
			if (!groups.has(r.eventNumber))
				groups.set(r.eventNumber, {
					eventNumber: r.eventNumber,
					date: r.date,
					runners: [],
					vols: [],
			...
			<div class={styles.page}>
				...
				<FieldBlock class={styles.header} title={eventName()} signType="purple">
					...
					<div class={styles.statsGrid}>
						<RunnerSummaryStat label="🗓️ Events Here">
							{totalVisits()}
						</RunnerSummaryStat>
						<RunnerSummaryStat label="👥 Members Involved">
							{totalParticipantIds().size}
						</RunnerSummaryStat>
						<RunnerSummaryStat label="🏃 Total Finishes">
							{eventResults().length}
						</RunnerSummaryStat>
						<RunnerSummaryStat label="📏 KMs Covered">
							{totalKm()}
						</RunnerSummaryStat>
					</div>
				</FieldBlock>
				...
				<DirtBlock title="Hall of Fame">
					<div class={styles.awards}>
							...
							{(() => {
								...
								return (
									<div class={styles.awardCard}>
										...
										<div class={styles.awardTitle}>
											{tied.length > 1 ? 'The Regulars' : 'The Regular'}
										</div>
										<div class={styles.awardValue}>{maxCount} runs</div>
										<div class={styles.awardDetail}>
											<span class={styles.buddyFaces}>
												<For each={tied}>
													{(r) => runnerFaceTooltip(r.parkrunId, r.name)}
												</For>
											</span>
										</div>
									</div>
							...
							{(() => {
								...
								return (
									<div class={styles.awardCard}>
										<div class={styles.awardEmoji}>⭐</div>
										<div class={styles.awardTitle}>
											{tied.length > 1 ? 'Volunteer Stars' : 'Volunteer Star'}
										</div>
										...
										<div class={styles.awardDetail}>
											...
											<br />
											<span class={styles.awardMeta}>
												{totalRoles.size} unique role
												{totalRoles.size !== 1 ? 's' : ''}
											</span>
										</div>
									</div>
						...
						<Show
							when={
								longestStreak().count > 1 && longestStreak().holders.length > 0
							}
						>
							<div class={styles.awardCard}>
								<div class={styles.awardEmoji}>🔥</div>
								<div class={styles.awardTitle}>
									{longestStreak().holders.length > 1
										? 'Streak Champions'
										: 'Streak Champion'}
								</div>
								<div class={styles.awardValue}>
									{longestStreak().count} weeks
								</div>
								<div class={styles.awardDetail}>
									<span class={styles.buddyFaces}>
										<For each={longestStreak().holders}>
											{(id) => runnerFaceTooltip(id, id)}
										</For>
									</span>
								</div>
							</div>
						...
						<Show when={theOGs().length > 0}>
							<div class={styles.awardCard}>
								<div class={styles.awardEmoji}>👑</div>
								<div class={styles.awardTitle}>
									{theOGs().length > 1 ? 'The OGs' : 'The OG'}
								</div>
								<div class={styles.awardValue}>#{theOGs()[0].eventNumber}</div>
								<div class={styles.awardDetail}>
									<span class={styles.buddyFaces}>
										<For each={theOGs()}>
											{(og) => runnerFaceTooltip(og.parkrunId, og.name)}
										</For>
									</span>
									<br />
									<span class={styles.awardMeta}>
										First club member{theOGs().length > 1 ? 's' : ''} here
									</span>
								</div>
							</div>
						</Show>
				...
				<DirtBlock title="Fun Facts">
					<div class={styles.funFacts}>
						<Show when={firstVisit()}>
							<div class={styles.factItem}>
								...
								<span>
									First club visit:{' '}
									<strong>
										{formatDate(new Date(`${firstVisit()}T00:00:00`))}
									</strong>{' '}
									— that's <strong>{daysSinceFirst()} days ago!</strong>
								</span>
							</div>
						</Show>
						<Show when={lastVisit()}>
							<div class={styles.factItem}>
								...
								<span>
									Most recent visit:{' '}
									<strong>
										{formatDate(new Date(`${lastVisit()}T00:00:00`))}
									</strong>
								</span>
							</div>
						</Show>
						...
						<div class={styles.factItem}>
							<span class={styles.factEmoji}>📆</span>
							<span>
								We've been here across{' '}
								<strong>{uniqueMonths()} different months</strong>
							</span>
						</div>
						<Show when={isJunior()}>
							<div class={styles.factItem}>
								<span class={styles.factEmoji}>🧒</span>
								<span>
									<strong>Junior event!</strong> parkrun for the little legends
								</span>
							</div>
						</Show>
						<Show when={!isJunior() && nonSaturdayDays().length > 0}>
							<div class={styles.factItem}>
								<span class={styles.factEmoji}>📅</span>
								<span>
									Joined on a <strong>{nonSaturdayDays().join(' & ')}</strong>{' '}
									special event{nonSaturdayDays().length > 1 ? 's' : ''}!
								</span>
							</div>
						</Show>
						<Show when={eventNumberRange().span > 1}>
							<div class={styles.factItem}>
								...
								<span>
									We've spanned{' '}
									<strong>{eventNumberRange().span} event numbers</strong> (#
									{eventNumberRange().min} → #{eventNumberRange().max})
								</span>
							</div>
						</Show>
						...
						<Show when={uniqueRoles().length > 0}>
							<div class={styles.factItem}>
								<span class={styles.factEmoji}>🎭</span>
								<span>
									We've covered <strong>{uniqueRoles().length}</strong>{' '}
								...
								<span>
									...
									{totalKm() >= 42 && (
										<>
											...
											— over{' '}
											<strong>
												{Math.floor(totalKm() / 42)} marathon
												{Math.floor(totalKm() / 42) !== 1 ? 's' : ''}
											</strong>{' '}
											worth of running!
										</>
									)}
									...
									<For each={entry.vols}>
										{(v) => (
											<span class={styles.memberChip}>
												...
												<span class={styles.memberChipMeta}>
...
const styles = {
	...
	headerExtLink: css({
		...
		height: '24px',
		p: '4px',
		_hover: { background: 'var(--overlay-black-10)' },
	}),
