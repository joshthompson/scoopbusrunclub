...
export function ConnectionsPage(props: ConnectionsPageProps) {
	const edges = createMemo(() =>
		computeConnections(props.results, props.volunteers),
	)
	const nodes = createMemo(() => layoutNodes(edges()))
	const maxLogStrength = createMemo(() =>
		Math.max(1, Math.log(Math.max(1, ...edges().map((e) => e.strength)))),
	)
	...
	const [selectedEdge, setSelectedEdge] = createSignal<ConnectionEdge | null>(
		null,
	)
	...
	function nodeOpacity(n: NodeInfo) {
		...
		const connected = edges().some(
			(e) =>
				(e.idA === active && e.idB === n.id) ||
				(e.idB === active && e.idA === n.id),
		)
		...
		<div class={pageStyles.page}>
			<FieldBlock title="Connection Web">
				<p class={pageStyles.intro}>
					How connected is Scoop Bus Run Club? Every shared parkrun, close
					finish, and volunteer session weaves the web tighter.
				</p>
				<div class={pageStyles.formulaBlock}>
					<div class={pageStyles.formulaLabel}>
						<em>"Real science"</em>
					</div>
					<div class={pageStyles.formulaText}>
						Connection Strength = (Runs + Volunteers + Close Finishes) × Unique
						Events
					</div>
				</div>
					...
					<svg viewBox="-30 -30 580 560" class={pageStyles.svg}>
						...
						<For each={nodes()}>
							{(node) => {
								...
								const dyShift =
									ny < -0.3 ? '-0.3em' : ny > 0.3 ? '1em' : '0.35em'
								return (
									<g
								...
								return (
									<div class={pageStyles.hoverRow}>
										...
										<span class={pageStyles.hoverFormula}>
											({e.sharedEvents} + {e.volunteeredTogether} +{' '}
											{e.closeFinishes}) × {e.uniqueEvents} = {e.strength}
										</span>
									</div>
								)
							}}
			...
			<DirtBlock title="Strongest Connections">
				<Table
					columns={[
						{ title: 'Connection' },
						{
							title: '(Runs + Volunteers + Finishes) × Unique',
							width: '300px',
						},
