...
const TAG_COLORS = {
	pb: 'var(--blue-600)',
	juniorPb: 'var(--pink-hot)',
	coursePb: 'var(--green-600)',
	milestone: 'var(--pink-600)',
	...
	goteborgDouble: 'var(--orange-600)',
	birthday: 'var(--pink-rose)',
	spellingBus: 'var(--amber-600)',
	aloneHaga: 'var(--purple-violet)',
	hagaCancelled: 'var(--blue-cyan)',
	uppsalaCancelled: 'var(--blue-indigo)',
	palindrome: 'var(--green-teal)',
	runBuddy: 'var(--orange)',
	bestie: 'var(--purple-soft)',
	bff: 'var(--pink-soft)',
	...
	volunteerMilestone: 'var(--purple-violet)',
	runDirector: 'var(--red-600)',
} as const
 ...
 */
function buildEventListMap(
	results: RunResultItem[],
): Map<string, EventListAchievement[]> {
...
const SPELLING_ACHIEVEMENTS: SpellingAchievement[] = [
	{
		...
		description:
			'Spell SCOOP BUS with parkrun event first letters (duplicate letters need duplicate runs)',
 ...
 */
function buildSpellingMap(
	results: RunResultItem[],
): Map<string, SpellingAchievement> {
...
function buildAloneAtHagaMap(results: RunResultItem[]): Set<string> {
	...
	const sortedEvents = [...hagaByEvent.entries()].sort((a, b) =>
		...
		for (let i = 0; i < eventResults.length; i++) {
			for (let j = i + 1; j < eventResults.length; j++) {
				...
				if (!earnedBuddy.has(pairKey)) {
					...
					const exA = runBuddyMap.get(keyA) ?? []
					exA.push({ name: nameB, parkrunId: b.parkrunId })
					runBuddyMap.set(keyA, exA)
					const exB = runBuddyMap.get(keyB) ?? []
					exB.push({ name: nameA, parkrunId: a.parkrunId })
					runBuddyMap.set(keyB, exB)
				}
				...
				if (count === 10 && !earnedBestie.has(pairKey)) {
					...
					const nameA = nameMap.get(a.parkrunId) ?? a.runnerName
					const nameB = nameMap.get(b.parkrunId) ?? b.runnerName
					const exA = bestieMap.get(keyA) ?? []
					exA.push({ name: nameB, parkrunId: b.parkrunId })
					bestieMap.set(keyA, exA)
					const exB = bestieMap.get(keyB) ?? []
					exB.push({ name: nameA, parkrunId: a.parkrunId })
					bestieMap.set(keyB, exB)
				}
				...
				if (count === 50 && !earnedBFF.has(pairKey)) {
					...
					const nameA = nameMap.get(a.parkrunId) ?? a.runnerName
					const nameB = nameMap.get(b.parkrunId) ?? b.runnerName
					const exA = bffMap.get(keyA) ?? []
					exA.push({ name: nameB, parkrunId: b.parkrunId })
					bffMap.set(keyA, exA)
					const exB = bffMap.get(keyB) ?? []
					exB.push({ name: nameA, parkrunId: a.parkrunId })
					bffMap.set(keyB, exB)
				}
			}
		...
		for (let i = 0; i < runnerList.length; i++) {
			for (let j = i + 1; j < runnerList.length; j++) {
				...
				if (count === 50 && !earnedPal.has(pairKey)) {
					...
					const nameA = nameMap.get(a.parkrunId) ?? a.runnerName
					const nameB = nameMap.get(b.parkrunId) ?? b.runnerName
					const exA = map.get(keyA) ?? []
					exA.push({ name: nameB, parkrunId: b.parkrunId })
					map.set(keyA, exA)
					const exB = map.get(keyB) ?? []
					exB.push({ name: nameA, parkrunId: a.parkrunId })
					map.set(keyB, exB)
				}
			}
 ...
 */
function buildPalindromePalMap(
	results: RunResultItem[],
): Map<string, PairPartner[]> {
	const map = new Map<string, PairPartner[]>()
	const nameMap = buildRunnerNameMap()
	...
	for (const eventResults of byEvent.values()) {
		...
		for (const entry of digitEntries) {
			...
			const reversedSecs = Number.parseInt(reversed.slice(2), 10)
		...
		for (let i = 1; i < hagaRuns.length; i++) {
			...
			if (diffMs === ONE_WEEK_MS) {
				...
				if (streak >= STREAK_LENGTH && streak % STREAK_LENGTH === 0) {
					...
					set.add(
						`${run.parkrunId}:${run.date}:${run.event}:${run.eventNumber}`,
					)
				}
			} else {
...
export function buildCelebrationData(
	results: RunResultItem[],
	runners: Runner[],
...
/** "parkrunId:date:event:eventNumber" → volunteer count (only for debuts & milestones) */
function buildVolunteerMilestoneMap(
	volunteers: VolunteerItem[],
): Map<string, number> {
 ...
 */
export function getOrBuildCelebrationData(
	results: RunResultItem[],
	runners: Runner[],
	volunteers?: VolunteerItem[],
): CelebrationData {
...
const celebrationRules: ((
	ctx: CelebrationRuleContext,
) => CelebrationTag | CelebrationTag[] | null)[] = [
	...
	({ data, resultKey }) => {
		...
		if (pb.firstRun)
			return {
				label: 'Parkrun debut!',
				description: 'First ever parkrun result recorded',
				emoji: TAG_EMOJIS.debut,
				color: TAG_COLORS.pb,
			}
		...
		if (pb.juniorPb) {
			tags.push({
				label: 'New Junior PB!',
				description: 'New personal best time at a junior parkrun',
				emoji: TAG_EMOJIS.juniorPb,
				color: TAG_COLORS.juniorPb,
			})
		}
		...
		if (pb.pb) {
			tags.push({
				label: 'New PB!',
				description: 'New overall personal best time',
				emoji: TAG_EMOJIS.pb,
				color: TAG_COLORS.pb,
			})
		}
		...
		if (pb.coursePb) {
			...
			tags.push({
				label: `New ${eventDisplayName} PB!`,
				description: `New personal best time at ${eventDisplayName}`,
				emoji: TAG_EMOJIS.coursePb,
				color: TAG_COLORS.coursePb,
			})
		}
	...
	({ data, resultKey }) =>
		data.haga1Map.has(resultKey)
			? {
					label: 'Haga Debut',
					description: 'First run at Haga park!',
					emoji: TAG_EMOJIS.haga1,
					color: TAG_COLORS.haga1,
				}
			: null,
	...
	({ data, resultKey }) =>
		data.haga100Map.has(resultKey)
			? {
					...
					emoji: TAG_EMOJIS.haga100,
					color: TAG_COLORS.haga100,
				}
			: null,
	...
	({ data, parkrunId, date }) => {
		...
		return `${dd}/${mm}` === birthday
			? {
					label: 'Birthday Runner!',
					description: 'Ran a parkrun on their birthday',
					emoji: TAG_EMOJIS.birthday,
	...
	({ data, resultKey }) =>
		data.hagaCancelledMap.has(resultKey)
			? {
					label: 'Haga Cancelled?',
					description:
						'Ran Uppsala instead of Haga during the winter - was Haga cancelled due to ice?',
					emoji: TAG_EMOJIS.hagaCancelled,
					color: TAG_COLORS.hagaCancelled,
				}
			: null,
	...
	({ data, resultKey }) =>
		data.uppsalaCancelledMap.has(resultKey)
			? {
					label: 'Uppsala Cancelled!?!',
					description:
						'Both Haga and Uppsala were cancelled? Had to flee to Djäkneberget or Örebro!',
					emoji: TAG_EMOJIS.uppsalaCancelled,
					color: TAG_COLORS.uppsalaCancelled,
				}
			: null,
	...
	({ data, resultKey }) =>
		data.palindromeMap.has(resultKey)
			? {
					label: 'Palindrome!',
					description:
						'Finish with a time that reads the same forwards and backwards in mm:ss',
	...
	({ data, resultKey }) =>
		data.vikingMap.has(resultKey)
			? {
					label: 'Viking!',
					description:
						'Completed a parkrun in Sweden, Finland, Denmark and Norway',
...
export function getCelebrationTags(
	ctx: CelebrationRuleContext,
): CelebrationTag[] {
...
export function getVolunteerCelebrationTags(
	data: CelebrationData,
	parkrunId: string,
	date: string,
	eventId: string,
	...
	if (count === 1) {
		result.push({
			label: 'Volunteer debut!',
			description: 'First time volunteering at Haga',
			emoji: TAG_EMOJIS.volunteerDebut,
			color: TAG_COLORS.volunteerDebut,
		})
	} else if (count !== undefined) {
		result.push({
			label: `${ordinalSuffix(count)} volunteer!`,
			description: `Volunteered ${count} times at Haga`,
			emoji: TAG_EMOJIS.volunteerMilestone,
			color: TAG_COLORS.volunteerMilestone,
		})
	}
	...
	if (roles.includes('Loppansvarig')) {
		result.push({
			label: 'Run Director!',
			description: 'Call the shots at Haga Parkrun',
			emoji: TAG_EMOJIS.runDirector,
			color: TAG_COLORS.runDirector,
		})
	}
...
export function CelebrationPill(props: {
	tag: CelebrationTag
	showTooltip?: boolean
}) {
	...
	const otherRunnerFace = () =>
		props.tag.otherRunnerId ? getRunnerFace(props.tag.otherRunnerId) : undefined
