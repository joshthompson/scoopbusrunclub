...
function LightningBolt(props: {
	...
	const PX = 2
	const segs = generateBoltSegments(
		props.seed,
		Math.ceil(props.totalHeight / PX),
	)
	...
	return (
		...
		>
			...
			<g clip-path={`url(#bolt-clip-${props.seed})`}>{pixels}</g>
		</svg>
	) as JSX.Element
}

export function createBusController(
	id: string,
	scene: Scene,
	lightning = false,
) {
	...
	const bus = createController({
		...
		onEnterFrame({ $, $age, $scene }) {
			...
			if ($.x() < endX) {
				$.setX(
					Math.max(
						$scene.canvas.get().width() + 22 + MIN_BUS_OFFSCREEN_PX,
						MIN_BUS_START_X,
					),
				)
			}
	...
	if (lightning) {
		...
		boltPositions.forEach((pos, i) => {
			...
			let boltCooldown =
				BOLT_COOLDOWN_MIN +
				Math.floor(Math.random() * BOLT_COOLDOWN_VARY) +
				i * 3
					...
					init: () =>
						({
							children: () => {
								...
								const opacity =
									p === 'fade' ? Math.max(0, 1 - frame() / BOLT_FADE_FRAMES) : 1
						...
						if (p === 'idle') {
							...
							if (boltCooldown <= 0) {
								...
								setBoltHeight(
									BOLT_HEIGHT_MIN +
										Math.floor(
											Math.random() * (BOLT_HEIGHT_MAX - BOLT_HEIGHT_MIN),
										),
								)
							}
						} else if (p === 'reveal') {
