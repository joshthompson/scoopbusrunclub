...
export function Milestones(props: Props) {
	const groups = createMemo(() => {
		...
		for (const r of data) {
			if (
				MILESTONE_SET.has(r.totalRuns) &&
				latestEventRunnerIds.has(r.parkrunId)
			) {
				celebrated.push({
					name: r.name,
					parkrunId: r.parkrunId,
					milestone: r.totalRuns,
				})
			} else {
						...
						<For each={groups().celebrated}>
							{(row) => (
								<li class={styles.celebRow}>
									...
									{ordinalSuffix(row.milestone)} run!{' '}
									<Emoji emoji="🎉" shadow />
								</li>
							)}
						</For>
						...
						<For each={groups().upcoming}>
							{(row) => (
								<li class={styles.row}>
									...
									{row.next}
									<div class={styles.possibleDate}>
										Potentially {row.possibleDate}
									</div>
								</li>
							)}
						</For>
