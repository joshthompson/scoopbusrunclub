...
function RaceBlock(props: { race: RaceItem; guests: GuestItem[] }) {
	...
	const linkedName = (runnerId: string) => {
		...
		return (
			<em>
				<A href={`/member/${runnerId}`} class={styles.memberLink}>
					{name}
				</A>
			</em>
		)
	...
	return (
		<>
			<h4 class={styles.parkrunName}>
				...
				<A
					href={`/event/${props.parkrun.eventId}`}
					class={styles.parkrunNameLink}
				>
					{displayName()}
				</A>{' '}
				#{props.parkrun.eventNumber} {isBusTrip() && <Emoji emoji="🚌" />}
		...
		<Show when={image()}>
			{(img) => (
				<img
					src={img().src}
					alt={img().name}
					class={styles.parkrunImage}
					style={{ height: img().height || 'auto', width: 'auto' }}
				/>
			)}
		</Show>
	)
...
function translateRole(role: string): string {
	...
	if (!translations[role])
		console.warn(`No translation found for volunteer role: ${role}`)
...
function joinRoles(roles: string[]): JSX.Element {
	...
	if (roles.length === 2)
		return (
			<>
				<em>{roles[0]}</em> and <em>{roles[1]}</em>
			</>
		)
	return (
		<>
...
function ParkrunFlag(props: { parkrun: ParkrunEvent }) {
	...
	const flag = createMemo(() => {
		...
		if (ev?.country && ev.country !== 'SE' && countryFlags[ev.country])
			return countryFlags[ev.country]
		return null
	})
													...
													<For each={parkrun.volunteers}>
														{(vol) => {
															...
															const translatedRoles = () =>
																joinRoles(vol.roles.map(translateRole))
															return (
																<li>
																	<em>
																		<Show
																	...
																	volunteered as {translatedRoles()}
																	<VolunteerCelebrations
																		data={celebrations()}
																		parkrunId={vol.parkrunId}
																		date={result.date}
																		eventId={parkrun.eventId}
