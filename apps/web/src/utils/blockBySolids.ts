import type { ControllerBaseType } from './game'

export function blockBySolids(
	data: ControllerBaseType,
	prevX: number,
	prevY: number,
) {
	const x = data.x()
	...
	if (remainX !== 0 || remainY !== 0) {
		const { hit: hit2, t: t2 } = sweepSolids(
			midX,
			midY,
			remainX,
			remainY,
...
export function blockBySolidsRectPlayer(
	data: ControllerBaseType,
	prevX: number,
	prevY: number,
) {
	...
	if (remainX !== 0 || remainY !== 0) {
		let tSlideMin = 1
		for (const rect of expandedRects) {
			const res = sweepPointAgainstRect(
				player.x,
				player.y,
				remainX,
				remainY,
...
function sweepSolids(
	...
	return {
		hit: hitRect
			? { rect: hitRect, normalX: hitNormalX, normalY: hitNormalY }
			: null,
		t: tMin,
	}
}

function sweepPointAgainstRect(
	px: number,
	py: number,
	dx: number,
	dy: number,
	...
	let tEntry = Number.NEGATIVE_INFINITY
