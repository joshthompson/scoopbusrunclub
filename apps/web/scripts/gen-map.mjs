...
function httpGet(url) {
	return new Promise((resolve, reject) => {
		https
			.get(url, { headers: { 'User-Agent': 'node-gen-map/1.0' } }, (res) => {
				if (
					res.statusCode >= 300 &&
					res.statusCode < 400 &&
...
async function main() {
	...
	console.error(
		`Rasterising ${countryPolys.length} countries to ${COLS}×${ROWS} grid…`,
	)

	const grid = Array.from({ length: ROWS }, () => new Uint8Array(COLS))
	const countryGrid = Array.from({ length: ROWS }, () =>
					...
					for (const dlat of offsets) {
						...
						for (const country of countryPolys) {
							if (pointInMultiPolygon(slon, slat, country.polygons)) {
								hits++
								if (!bestCountry && country.parkrunCode)
									bestCountry = country.parkrunCode
								break
							}
						}
					}
