...
// --- Tracked athletes ---
// Re-exported from libs/shared so the web app can render the same list.

export {
	TRACKED_ATHLETES,
	TRACKED_IDS,
	type TrackedAthlete,
} from '../../../libs/shared/tracked-athletes'
	...
	for (let attempt = 1; attempt <= MAX_RETRIES + 1; attempt++) {
		...
		try {
			...
			if (attempt <= MAX_RETRIES) {
				...
				console.log(
					`  ⟳ Attempt ${attempt} failed, retrying in ${(backoff / 1000).toFixed(1)}s...`,
				)
				await sleep(backoff)
			} else {
				throw error
...
export async function getAppDataValue(
	...
	if (!response.ok) {
		console.warn(
			`Warning: failed to fetch appData key "${key}" (${response.status})`,
		)
		return null
	}
