...
async function main() {
	...
	console.log(
		`${existingSet.size} already have course data, ${toFetch.length} remaining.\n`,
	)
	...
	try {
		for (let i = 0; i < toFetch.length; i++) {
				...
				if (!result) {
					...
					if (!response.ok) {
						...
						results.push({
							eventId,
							status: 'error',
							error: `Ingest ${response.status}`,
						})
					} else {
						console.log(
							`  ✓ Stored (${courseData.coordinates.length} coords, ${courseData.points.length} points)`,
						)
			...
			if (i < toFetch.length - 1) {
				...
				console.log(
					`  ⏳ Waiting ${(delay / 1000).toFixed(0)}s before next event...`,
				)
				await sleep(delay)
			}
		}
